...
def build_stamp() -> str:
    """打包時間戳（`scripts/build.py` 產生），只給靜態檔快取破壞用（見
    `web/__init__.py` 的 `_asset_v`）——跟 `build_tag()` 不同，這個**不看 git
    commit 有沒有變**，同一個 commit 內反覆重新打包（發布前除錯迭代）也每次都
    不一樣，才能確保瀏覽器／CDN 快取真的被打破。開發模式沒有 `_build_info.py`
    → 回空字串。"""
    try:
        from bahaad._build_info import BUILD_STAMP
