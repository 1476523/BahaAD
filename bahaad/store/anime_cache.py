...
- `home_expiring_item`：首頁「即將下架」（N月份／N月上旬授權到期節目）卡片，一列一張，
  `group_label` 照站方原文分組、`group_order` 保留分組第一次出現的順序、`position` 是
  組內順序（2026-09-23 追加，跟上面三張表同一個 `home_cache_meta` 新鮮度、同一輪
  `save_home()` 整批換掉，不是獨立 TTL——使用者要求「只要查詢首頁時就一起抓」）。
...
CREATE TABLE IF NOT EXISTS home_expiring_item (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
class AnimeCacheStore:
    def __init__(self, database: Database) -> None:
        ...
        with self._database.transaction() as conn:
            ...
            # 使用者 2026-09-24 回報：升級到 v0.1.0 後「即將下架」頁一直空著——
            # `home_expiring_item` 是全新的表，舊資料庫升級上來後這張表一開始一定是
            # 空的，但 `home_cache_meta` 還留著升級前那輪抓的「新鮮」時間戳，
            # `get_home()` 判斷快取沒過期就不會觸發重抓，這張表因此沒機會被填。
            # 這裡不是只判斷「表格剛建立」（那樣的話已經升級過一次、表已經建立但還
            # 是空的使用者，下次再更新也不會被救到，使用者 2026-09-24 追問「那強制
            # 更新呢？」問的就是這個情境）——改成每次開機都檢查：這張表目前是空的、
            # 但 `home_cache_meta` 已經有資料（代表首頁本來就抓取過，只是那次抓取沒有
            # 「即將下架」這個欄位），就清掉 meta 強制下次開首頁重抓一次，真的抓到
            # 資料後這個條件就不會再成立，不會每次開機都白白重抓。
            has_expiring_rows = conn.execute("SELECT 1 FROM home_expiring_item LIMIT 1").fetchone() is not None
            has_home_meta = conn.execute("SELECT 1 FROM home_cache_meta LIMIT 1").fetchone() is not None
            if not has_expiring_rows and has_home_meta:
    ...
    def save_home(
        ...
        expiring_groups: list[ExpiringLicenseGroup] | None = None,
    ) -> None:
        """整批換掉——同一交易刪光再插入，中途壞掉會 rollback、不留半套資料。
        `expiring_groups` 可選（預設 None＝視同空清單）——既有呼叫端不用跟著改。"""
        now = now or datetime.now()
        with self._lock:
            with self._database.transaction() as conn:
                conn.execute("DELETE FROM home_card")
                conn.execute("DELETE FROM home_schedule_entry")
                conn.execute("DELETE FROM home_expiring_item")
                conn.executemany(
                ...
                expiring_rows = []
                for group_order, group in enumerate(expiring_groups or []):
                    for pos, item in enumerate(group.items):
                        expiring_rows.append(
                            (
                                group.label, group_order, pos, item.ref_sn, item.title,
                                item.cover_url, item.year_text, item.episode_count_text,
                            )
                        )
                conn.executemany(
                    "INSERT INTO home_expiring_item "
                    "(group_label, group_order, position, ref_sn, title, cover_url, year_text, "
                    "episode_count_text) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    expiring_rows,
                )
                conn.execute(
    ...
    def get_expiring_groups(self) -> list[ExpiringLicenseGroup]:
        """首頁「即將下架」快取——沒有快取（從沒抓過首頁）回空清單，不是 None（呼叫端
        不用另外判斷 None，跟 `get_home()` 用 `None` 表示「完全沒快取過」的慣例不同，
        因為這個區塊本來就可能真的是空的——沒有任何授權到期節目時站方也不會有這個
        區塊）。"""
        with self._database.transaction() as conn:
            rows = conn.execute(
                "SELECT * FROM home_expiring_item ORDER BY group_order, position"
            ).fetchall()
        ...
        return [ExpiringLicenseGroup(label=label, items=groups[label]) for label in order]
    ...
    def get_detail_with_sibling_fallback(self, video_sn: int) -> tuple[AnimeDetail, datetime] | None:
        """`video_sn` 本身沒有個別快取過時，借用 `episode_group` 記錄的「同一部番劇
        其他有快取的集數」——動畫瘋回的是整季集數清單，抓哪一集的詳細頁內容都一樣
        （原本只有 `web/anime_data.py._sibling_group_detail()` 這個公開模式專用版本，
        使用者 2026-09-17 回報：番劇更新通知有時只有集數封面、沒有番劇封面，根因是
        通知路徑〔`subscriber/dispatch.py`／`scheduler/main_loop.py`〕直接呼叫裸的
        `get_detail()`，沒有這層 fallback——搬進這裡讓兩邊共用同一套邏輯）。找不到
        群組或群組內沒有任何一筆有快取就回 `None`。"""
        hit = self.get_detail(video_sn)
        if hit is not None:
            return hit
        group_key = self.group_key_for(video_sn)
        if group_key is None:
            return None
        for member_sn in sorted(self.group_members(group_key)):
            if member_sn == video_sn:
                continue
            hit = self.get_detail(member_sn)
            if hit is not None:
                return hit
        return None
    ...
    def _referenced_image_urls(conn) -> set[str]:
        ...
        for sql in (
            ...
            "SELECT cover_url AS u FROM home_expiring_item",
    ...
    def clear(self) -> None:
        ...
        with self._lock:
            with self._database.transaction() as conn:
                for table in (
                    "home_card", "home_schedule_entry", "home_cache_meta", "home_expiring_item",
