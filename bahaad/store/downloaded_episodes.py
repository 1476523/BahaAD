...
class DownloadedEpisodeStore:
    ...
    def list_for_migration(self) -> list[dict]:
        """更改下載目錄時的遷移候選清單（`scheduler/download_dir_migration.py` 用）：
        目前「還算已下載」（沒被標記移除）的每一筆，帶 `video_sn`／`file_path`。哪些
        真的需要搬（還在舊目錄底下）交給呼叫端自己判斷——這裡不預設任何目錄字串，
        單純是資料庫現況的一份快照。規格見 `docs/requirements/download_dir_migration.md`。"""
        with self._database.transaction() as conn:
            rows = conn.execute(
                "SELECT video_sn, file_path FROM downloaded_episodes WHERE removed_at IS NULL"
            ).fetchall()
        return [{"video_sn": row["video_sn"], "file_path": row["file_path"]} for row in rows]

    def get_file_path_raw(self, video_sn: int) -> str | None:
        """純讀 `file_path`，不檢查檔案存不存在、不做 `removed_at` 副作用——遷移收尾時
        核對「資料庫真的切到新路徑了嗎」要看的是這個當下的真實值，不能用
        `playable_path()`／`is_downloaded()` 那種會順手判斷「檔案不在就標記移除」的
        版本（會混淆兩件不同的事）。"""
        with self._database.transaction() as conn:
            row = conn.execute(
                "SELECT file_path FROM downloaded_episodes WHERE video_sn = ? AND removed_at IS NULL",
                (video_sn,),
            ).fetchone()
        return row["file_path"] if row is not None else None

    def update_file_path(self, video_sn: int, new_path: str) -> bool:
        """遷移單筆複製＋驗證通過後，把 `file_path` 切到新路徑。只更新這個欄位——
        不動 `downloaded_at`／`removed_at` 等其他狀態。回傳有沒有真的改到一筆（`video_sn`
        不存在或已被標記移除都回 `False`，遷移端據此判斷這筆是不是「途中被使用者動過」）。"""
        with self._lock:
            with self._database.transaction() as conn:
                cursor = conn.execute(
                    "UPDATE downloaded_episodes SET file_path = ? "
                    "WHERE video_sn = ? AND removed_at IS NULL",
                    (new_path, video_sn),
                )
