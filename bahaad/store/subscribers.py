...
class SubscribersStore:
    ...
    def create_session(self, identity_id: int, ttl_seconds: float = _SESSION_TTL_SECONDS) -> str:
        ...
        with self._database.transaction() as conn:
            conn.execute(
                "INSERT INTO subscriber_sessions (token_hash, subscriber_identity_id, created_at, expires_at) "
                "VALUES (?, ?, ?, ?)",
                (_hash_token(token), identity_id, now, now + ttl_seconds),
            )
        return token

    def resolve_session(self, token: str) -> dict[str, Any] | None:
        """回傳對應的訂閱者身分——**已經解析過帳號連結**，回傳的一律是主要身分
        （見 `resolve_primary_identity()`）。這樣不管訂閱者是從哪個當初登入時的
        管道（登入 session 綁的是那次登入當下的身分 id）重新整理頁面，看到的都
        是連結後同一份訂閱清單／通知偏好，不用另外處理「session 指到已經被併入
        別人的舊身分」這種情況。token 不存在／過期回 `None`。"""
        with self._database.transaction() as conn:
            row = conn.execute(
                "SELECT subscriber_identity_id, expires_at FROM subscriber_sessions WHERE token_hash = ?",
                (_hash_token(token),),
            ).fetchone()
        if row is None or row["expires_at"] < time.time():
            return None
        return self.resolve_primary_identity(row["subscriber_identity_id"])
