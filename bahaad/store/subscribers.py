...
_IDENTITY_ADDED_COLUMNS = {
    ...
    # 「發送測試訊息」驗證碼確認過的時間（使用者 2026-09-23）——NULL＝這個管道還沒
    # 驗證過。取代原本的 `has_successful_notify()`（只看中繼 API 有沒有回應成功）：
    # 使用者要求要真的輸入訊息裡附的驗證碼才算數，不是單靠 API 回應「送出成功」
    # 這種系統偵測。見 `subscriber/verify.py`、`web/subscriber_auth.py` 的
    # `notify_test()`／`notify_test_confirm()`。
    "verified_at": "TEXT",
}
...
class SubscribersStore:
    ...
    def get_identity(self, identity_id: int) -> dict[str, Any] | None:
        with self._database.transaction() as conn:
            row = conn.execute(
                "SELECT id, channel, external_id, display_name, linked_to, notify_channels, verified_at "
                "FROM subscriber_identities WHERE id = ?",
                (identity_id,),
            ).fetchone()
        return dict(row) if row is not None else None

    def is_verified(self, identity_id: int) -> bool:
        """這個（單一管道的）身分 id 有沒有透過「發送測試訊息」的驗證碼流程確認過
        （見 `mark_verified()`）——`me()` 拿來判斷「請先傳一次 /start」之類的提醒
        還要不要繼續顯示。"""
        identity = self.get_identity(identity_id)
        return identity is not None and identity.get("verified_at") is not None

    def mark_verified(self, identity_id: int) -> None:
        """驗證碼核對成功時呼叫（`web/subscriber_auth.py` 的 `notify_test_confirm()`）。"""
        with self._lock:
            with self._database.transaction() as conn:
                conn.execute(
                    "UPDATE subscriber_identities SET verified_at = datetime('now','localtime') WHERE id = ?",
                    (identity_id,),
                )
    ...
    def linked_identities_for(self, primary_id: int) -> list[dict[str, Any]]:
        ...
        with self._database.transaction() as conn:
            rows = conn.execute(
                "SELECT id, channel, external_id, display_name, linked_to, notify_channels, verified_at "
                "FROM subscriber_identities WHERE linked_to = ?",
                (primary_id,),
            ).fetchall()
    ...
    def unlink_identity(self, primary_id: int, other_id: int) -> None:
        """解除 `link_identity()` 建立的連結（使用者 2026-09-23：「帳號連接的部分
        加入移除帳號連接的選項」）。`other_id` 恢復成獨立身分，不會拿回連結當下
        被清空的那份舊訂閱清單——那份資料已經併入 `primary_id`（見上面
        `link_identity()` 的已知簡化），訂閱者要移除連結後重新自己收藏。
        `primary_id` 的 `notify_channels` 把這個管道移掉；移除後如果變成空集合
        （唯一連結的管道被移除這種邊界狀況），退回只留主要身分自己的管道，不留下
        「一個管道都沒開」的狀態。`other_id` 的 `verified_at` 一併清掉（使用者
        2026-09-23：「移除第三方帳號後新增的帳號也需要驗證」）——不這樣清的話，
        同一個 (channel, external_id) 之後重新連結時 `upsert_identity()` 是
        `ON CONFLICT DO UPDATE`（同一列），舊的驗證紀錄會原封不動留著，等於繞過
        了重新驗證。"""
        primary = self.get_identity(primary_id)
        other = self.get_identity(other_id)
        if primary is None or other is None or other.get("linked_to") != primary_id:
            return
        channels = self.get_notify_channels(primary_id) - {other["channel"]}
        if not channels:
        ...
        with self._lock:
            with self._database.transaction() as conn:
                conn.execute(
                    "UPDATE subscriber_identities SET linked_to = NULL, notify_channels = NULL, "
