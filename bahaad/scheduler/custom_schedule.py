...
# 時 aniGamerPlus 已經抓第 9 集）。觸發後在這個時窗內每分鐘再查一次，抓到新集數（或
# 時窗到）就停（使用者定：每分鐘、持續 10 分鐘）。
_EPISODE_RETRY_WINDOW = timedelta(minutes=10)
_EPISODE_RETRY_INTERVAL = timedelta(minutes=1)
...
class CustomScheduleRunner:
        ...
        for entry in entries:
            ...
            if submitted:
                ...
                self._episode_retries[entry.sn] = (
                    now + _EPISODE_RETRY_INTERVAL, now + _EPISODE_RETRY_WINDOW,
                )
    ...
    def _process_due_episode_retries(self, entries: list[ScheduleEntry]) -> None:
        ...
        for sn, (next_at, window_end) in list(self._episode_retries.items()):
            if next_at > deadline:
                continue
            if now > window_end or sn not in by_sn:
            ...
            if submitted:
                ...
                self._episode_retries[sn] = (now + _EPISODE_RETRY_INTERVAL, window_end)
