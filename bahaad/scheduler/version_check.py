...
2026-09-12：拿掉了額外推播 Telegram/Discord「有新版本可用」通知那段——上方已經有
專門的更新橫幅 UI（`_on_new_version` 回呼），兩邊重複通知使用者，改成只保留橫幅。
"""
...
# 檢查週期固定 10 分鐘，不開放自訂（使用者 2026-08-29 定案 1 小時；使用者 2026-09-24
# 改成 10 分鐘，一樣寫死不開放自訂）——只是打一次 GitHub Releases API、成本極低，
# 沒有讓使用者調的意義。10 分鐘一次遠低於 GitHub 未授權請求的每小時 60 次上限。
...
class VersionChecker:
    ...
    def _run_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            self._stop_event.wait(_INTERVAL_MINUTES * 60)
