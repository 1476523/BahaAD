...
class VersionChecker:
    ...
    def _fetch_latest_release(self) -> dict | None:
        ...
        if self._include_prereleases():
            releases = self._http.get(_GITHUB_RELEASES_LIST_URL).json()
            if not isinstance(releases, list) or not releases:
                return None
            return releases[0]
        return self._http.get(_GITHUB_RELEASES_URL).json()
    ...
    def check_once(self) -> str | None:
        ...
        if is_new_detection and self._notify_store is not None:
            body = _truncate_body(str(payload.get("body") or ""))
            send_notification(
                self._notify_store, self._settings, self._http, "system_new_version",
                version_tag=latest_version, version_body=body,
            )
