...
使用者 2026-09-18：三種季度分類（只影響檢查頻率，抓取／解析邏輯三者共用同一套）：
- **當季**（含跨季延續播出）：當季 + 前 2 季，每輪（每小時）都檢查。
- **新番**：下一季，還沒開播，每 24h 檢查一次。
- **舊番**：當季窗口以外、涵蓋 youranimes.tw 建站以來全部季度，每 24h 檢查一次。

搜尋（`youranimes.tw/search`）不能拿來補查任意番劇——2026-09-18 實測確認：搜尋結果
是額外一個 Next.js RSC 請求另外組出來的，`curl_cffi` 抓到的原始 HTML 沒有結果資料，
只能靠季度頁本身涵蓋。
...
from bahaad.youranimes.season import youranimes_next_season_slug, youranimes_season_slugs

logger = logging.getLogger(__name__)

# 檢查週期固定 1 小時，不開放自訂（比照 version_check）
_INTERVAL_HOURS = 1
# 「當季」類別（含跨季延續播出）：當季 + 前 2 季，每輪都檢查
_CURRENT_WINDOW_COUNT = 3
# 「舊番」類別的總涵蓋深度：涵蓋 youranimes.tw 建站（2020-06）以來全部季度（使用者
# 2026-09-18：「全季」），含「當季」窗口在內算出來的總季數上限——多算幾季不會出錯，
# 太早、還沒建站的 slug 抓回來只是空 parse（見 check_once() 的空 parse 處理，不覆蓋
# 既有資料，只記一次 warning）。
_HISTORY_SEASON_COUNT = 30
# 「新番」（下一季）／「舊番」的季度頁改動很慢，隔這麼久才重抓一次；只有「當季」
# 窗口每輪都查
...
class YourAnimesSync:
    ...
    def check_once(self) -> None:
        today = self._clock()
        # 使用者 2026-09-18：三種季度分類，只影響「多久檢查一次」——
        # 「當季」（含跨季延續播出）：當季 + 前 _CURRENT_WINDOW_COUNT-1 季，每輪都查；
        # 「新番」：下一季，還沒開播，資料變動不頻繁；
        # 「舊番」：當季窗口以外、涵蓋 youranimes.tw 建站以來全部季度的其餘歷史季度。
        # 新番／舊番共用同一種「每 24h 才查一次」節奏，不需要為了這個分類另外加一種
        # 頻率——抓取／解析／防洗掉（`_carry_over_backfilled`）都是同一套邏輯，差別
        # 只在 `_due()` 那一格 `is_current` 的判斷。
        current_slugs = youranimes_season_slugs(today, _CURRENT_WINDOW_COUNT)
        old_slugs = youranimes_season_slugs(today, _HISTORY_SEASON_COUNT)[_CURRENT_WINDOW_COUNT:]
        plan: list[tuple[str, bool]] = [(slug, True) for slug in current_slugs]
        plan.append((youranimes_next_season_slug(today), False))
        plan.extend((slug, False) for slug in old_slugs)

        for slug, is_current in plan:
            if not self._due(slug, is_current=is_current):
                continue
            try:
