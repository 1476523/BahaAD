...
from bahaad.notify.render import render_message
...
def split_clauses(text: str) -> list[str]:
    ...
    使用者 2026-09-26 回報真實案例：「本週《拉拉熊》暫停更新、《我家的弟弟們》將播出
    特別節目」，用頓號「、」並列兩部完全不同作品的處置，卻被當成同一句處理——
    `has_pause` 是整句共用的旗標，《我家的弟弟們》那句雖然實際上跟暫停無關（後來證實
    是要播特別集數，站方原始頁面確實多了一集 12.5），也被錯誤歸類成「本週暫停更新」。
    加一條「頓號後面緊接著另一個《作品名》才拆」的規則修正：只有『、《』這個組合才算
    分隔兩部作品的邊界，單純列舉同一部作品的集數（「《膽大黨》第10話、第11話將於
    20:00 同時更新」）用頓號串起集數清單、後面不是另一個《作品》，不會被誤拆，`第11話`
    仍然跟第 10 話同一句、同一組判斷。"""
...
def classify_status(
    ...
    # 「暫停」「下架」公告本身不帶時刻，`resolve_date(None, now)` 只會回 `now` 當天的
    # 日曆日期——對深夜番（凌晨播出、`schedule_weekday` 存的是「隔天」）來說，公告若在
    # 播出時刻之後才處理，`target_date` 會停在今天，跟 `_digest_content()` 判斷「這筆
    # 覆蓋算不算今天播出日」用的 13:00 分界對不起來，導致這筆機動調整從每日彙整消失
    # （使用者 2026-09-23 回報：暫停更新的項目沒有出現在重發的彙整通知裡）。公告沒寫
    # 明確日期、但這部有自訂時段時，改用 `baseline_time`（下一次正常播出的日期）取代
    # `now` 當天，跟 `_digest_content()` 的播出日認定一致。
    if action == "takedown":
        d = baseline_time.date() if (event.explicit_date is None and baseline_time) else resolve_date(event.explicit_date, now)
        return ClassifiedStatus("暫時下架", d, None)
    if action == "pause":
        d = baseline_time.date() if (event.explicit_date is None and baseline_time) else resolve_date(event.explicit_date, now)
        return ClassifiedStatus("暫停更新", d, None)
...
# 合併通知（`_notify_gossip_batch`）每部作品那一行的動詞，跟 `notify/categories.py`
# `DEFAULT_TEMPLATES` 裡各分類單則範本的第一行用字一致，只是拿掉了範本裡固定的
# 「《名稱》」外殼、改成程式組字串時逐一套用。
_BATCH_ACTION_VERB = {
    "pause": "本週停止更新",
    "takedown": "暫時下架",
...
class GossipWatcher:
    ...
    def _scan_gossip(self, now: datetime, *, force: bool) -> None:
        ...
        if text:  # 有公告文字才解析
            ...
            if source_hash is not None:  # 第一次出現、或 force
                ...
                # 同一則公告底下的子事件收集起來，處理完整份公告後一次送出合併通知
                # （`_notify_gossip_batch`）——不逐一各發一則（使用者 2026-09-23 定案）。
                ...
                for sub in parse_gossip_text(text):
                    ...
                    if skip_title is not None and (
                        ...
                        continue
                    try:
                        result = self._process_sub_event(sub, source_hash, now, dynamic_on, tracked)
                        if result is not None:
                            notify_items.append(result)
                    except Exception:
                        logger.exception("處理公告子事件時發生未預期的例外：%s", sub.raw_clause)
                try:
                    self._notify_gossip_batch(notify_items, text, now)
                except Exception:
                    logger.exception("發送公告合併通知時發生未預期的例外")
    ...
    def _process_sub_event(self, sub, source_hash, now, dynamic_on, tracked):
        """回傳這個子事件該併入這輪公告合併通知的資料（`(sub, status, entry, sn)`），
        或 `None`（沒有東西要通知——`log_only`／`unmatched`）。**不在這裡直接送通知**：
        一則公告常常同時提到好幾部作品，逐一各發一則使用者讀起來很亂、也不好跟原始公告
        對照（使用者 2026-09-23 要求），改成呼叫端（`_scan_gossip`）收集同一則公告底下
        全部子事件的結果，一次呼叫 `_notify_gossip_batch()` 合併成一則。"""
        ...
        if subscribed_sn is None:
            ...
            self._notify_subscribers_gossip(sub, status, None, now, sn=airing_sn)
            return (sub, status, None, airing_sn)
        ...
        # 有沒有開，公告通知都照樣送（使用者要知道的是公告內容本身）——送通知這件事
        # 交回給呼叫端批次處理，這裡只回傳資料。
        ...
        self._notify_subscribers_gossip(sub, status, entry, now)
        return (sub, status, entry, None)
    ...
    def _process_reschedule(self, notice: RescheduleNotice, source_hash, now, dynamic_on, tracked) -> None:
        ...
        **只處理一次**：同一部番、同一個新時段，只要 `gossip_events` 裡已經有一筆
        ...
        if subscribed_sn is None:
            ...
            self._notify_gossip(fake_sub, None, None, now, sn=airing_sn)
            return
    ...
    def _notify_gossip(self, sub, status: ClassifiedStatus, entry, now: datetime, sn: int | None = None) -> None:
        """單一子事件各自發一則（給不屬於「同一則公告一次處理完」的呼叫端用：
        `_recheck_no_subscription_events`／`_process_reschedule`／
        `_recheck_unmatched_reschedule_events`——這幾個本來就是零星、跨輪次觸發，沒有
        「同一批」可以合併）。`_scan_gossip` 主迴圈改用 `_notify_gossip_batch()`，
        見該方法說明。"""
        category = _NOTIFY_CATEGORY.get(sub.action, "gossip_other")
        stamp = now.strftime("%Y-%m-%d %H:%M:%S")
        clean_name = (entry.rename if entry is not None else None) or sub.title
        animation_name = f"{clean_name}（未訂閱）" if entry is None else clean_name
        postpone_time = _format_postpone(status) if sub.action == "delay" else None
        ...
        self._notify_subscribers_gossip(sub, status, entry, now, sn=sn)

    def _notify_subscribers_gossip(
        self, sub, status: ClassifiedStatus | None, entry, now: datetime, sn: int | None = None
    ) -> None:
        """訂閱者通知：跟擁有者的管理通知完全獨立判斷（不共用 `self._notify_store` 是否
        設定的早退），用 clean_name（不帶「（未訂閱）」——那是講給擁有者聽的，訂閱者
        看了會很困惑，訂閱者本人一定是有追蹤這部番才會收到）。`entry` 為 None 時
        （擁有者自己沒訂閱，但週期表上比對到了）用呼叫端傳進來的 sn；訂閱者是獨立於
        擁有者訂閱清單之外的另一群人，擁有者沒訂閱不代表沒有訂閱者在追這部番。

        擁有者通知不論逐一發還是合併發（見 `_notify_gossip_batch`），訂閱者這邊維持
        逐一——訂閱者只追蹤自己在意的那幾部，不會有「一次收到好幾部合併訊息」的
        閱讀負擔，沒有理由跟著改。"""
        category = _NOTIFY_CATEGORY.get(sub.action, "gossip_other")
        stamp = now.strftime("%Y-%m-%d %H:%M:%S")
        clean_name = (entry.rename if entry is not None else None) or sub.title
        postpone_time = _format_postpone(status) if status is not None and sub.action == "delay" else None
        resolved_sn = entry.sn if entry is not None else sn
        if self._subscriber_notifier is not None and resolved_sn is not None:
            try:
                self._subscriber_notifier.notify_gossip(
                    resolved_sn, category, clean_name, sub.raw_clause,
                    announcement_time=stamp, postpone_time=postpone_time,
                )
            except Exception:  # noqa: BLE001
                logger.exception("gossip_watch 發送訂閱者公告通知失敗（sn=%s）", resolved_sn)

    def _notify_gossip_batch(self, items: list[tuple], raw_text: str, now: datetime) -> None:
        """一則公告常常同時提到好幾部作品（使用者實例：「《蛀在糖糖裡》延至 00:30；
        《繼母與繼姐》延至 9/26 01:23 更新。《盜墓王》本週暫停更新。」），原本逐一各發
        一則，使用者讀起來拆得很散、也對不回原始公告。使用者 2026-09-23 定案：整份
        公告合併成一則，每部作品各一行「《名稱》狀態」開頭，後面接公告原文；要不要送
        看這則公告涉及的任一分類（`gossip_pause`／`gossip_delay`…）有沒有開，**只要有
        一個開，整份公告原文照樣完整送出**——不因為使用者只關掉其中一種分類，就把同一
        份公告拆成「送一半」，那樣反而更難讀、也做不到「原文照登」。

        `items` 是 `_process_sub_event()` 回傳值的清單（`None` 已在呼叫端濾掉），
        每筆 `(sub, status, entry, sn)`。"""
        if not items or self._notify_store is None:
            return
        categories = {_NOTIFY_CATEGORY.get(sub.action, "gossip_other") for sub, _status, _entry, _sn in items}
        enabled = [c for c in categories if category_enabled(self._settings, c)]
        if not enabled:
            return
        ...
        context: dict[str, object] = {
            "announcement_text": raw_text,
            "announcement_time": stamp,
            "finish_time": stamp,
        }
        for i, (sub, _status, entry, sn) in enumerate(items):
            clean_name = (entry.rename if entry is not None else None) or sub.title
            key = f"animation_name_{i}"
            context[key] = f"{clean_name}（未訂閱）" if entry is None else clean_name
            verb = _BATCH_ACTION_VERB.get(sub.action, _BATCH_DEFAULT_VERB)
            template_lines.append(f"《<b>@{key}@</b>》{verb}")
        template = "\n".join(template_lines) + "\n@announcement_text@"
        rendered = render_message(template, escape_html=True, **context)
        send_notification(
            self._notify_store, self._settings, self._http, enabled[0],
            prebuilt_html=rendered, finish_time=stamp,
        )
    ...
    def _maybe_send_daily_digest(self, now: datetime) -> None:
        ...
        # `_digest_content()` 只算一次，擁有者版跟訂閱者個人化版共用同一份結果——
        # 這裡面沒有像 `_resolve_anime_name` 那樣逐一包 try/except，任一筆訂閱項目在
        # 算名稱/預測集數時炸掉就會讓整支 `_maybe_send_daily_digest` 中途失敗，且
        # `_run_loop` 的 try/except 是包住整個 `check_once()`、不是只包這裡，例外會
        # 一路衝出去連 `_scan_gossip` 都跳過（使用者 2026-09-05 回報：整天一則每日
        # 彙整都沒收到）。先在這裡擋住，兩邊各自的發送再各自包一層，一邊失敗不牽連
        # 另一邊。
        try:
            schedule_lines, adjustment_lines, signature, sn_lines = self._digest_content(today)
        except Exception:  # noqa: BLE001
            logger.exception("每日新番彙整計算失敗（這一輪跳過，公告掃描不受影響）")
            return

        # 訂閱者的個人化每日新番通知：跟下面擁有者那份完全獨立判斷（不共用
        # `self._notify_store is None` 的早退，也不共用「今天已發送」狀態——每個
        # 訂閱者各自的日期去重在 SubscriberNotifier 內部處理，見 store/subscribers.py
        # 的 subscriber_digest_state）。這段每輪（~1 分鐘）都會跑，但每個訂閱者一天
        # 只會真的送出一次。
        if self._subscriber_notifier is not None:
            try:
                self._subscriber_notifier.notify_daily_digest(today, sn_lines)
            except Exception:  # noqa: BLE001
                logger.exception("每日新番通知（訂閱者個人化版）發送失敗")
        ...
        try:
            ...
            logger.exception("每日新番彙整發送失敗（這一輪跳過，公告掃描不受影響）")
    ...
    def _digest_content(
        self, today: date
    ) -> tuple[list[str], list[str], tuple[str, ...], list[tuple[int, str]]]:
        """回傳 `(排程清單文字, 機動調整清單文字, 變更偵測 signature, (sn, 該行文字) 清單)`。
        最後一項給訂閱者個人化每日新番通知用（`notify_daily_digest()`）——每個訂閱者
        只看得到自己追蹤的番劇對應的那幾行，不重新組一次名稱/集數/時間。
        ...
        **今天 13:00 ~ 隔天 12:59** 算同一個播出日（深夜番 23:00~01:00 更新、日曆已
        ...
        for pending in self._gossip_store.list_pending("pending"):
            ...
            adjustment_lines.append(
                f"- {self._pending_name(pending)} "
                f"{self._pending_time_change_text(pending) or self._pending_label(pending)}"
        ...
        schedule_lines = []
        sn_lines: list[tuple[int, str]] = []  # 訂閱者個人化每日新番通知用，見 notify_daily_digest()
        for hh, mm, sn, entry in rows:
            ...
            line = f"- {name} {ep_part}{hh:02d}:{mm:02d}"
            schedule_lines.append(line)
            sn_lines.append((sn, line))
        ...
        return schedule_lines, adjustment_lines, signature, sn_lines
    ...
    def _pending_time_change_text(self, pending: dict) -> str:
        """使用者 2026-09-24：每日新番通知的「機動調整異動」原本只寫「本週延後更新」
        這種籠統標籤，看不出改到幾點——希望直接寫「原更新時間 更改為 延後的時間」。
        只有真的有新時間可比的異動類型才適用（提前／延後／自訂更新時間／自訂連續
        更新時間／自訂時間範圍都有 `check_time`／`check_time_start`）；`skip_check`
        （本週暫停更新／下架）沒有新時間，回空字串讓呼叫端退回原本的 `_pending_label()`。
        「原更新時間」讀 `schedule_store` 目前登記的固定時段——這些是`gossip_pending`
        暫時性覆蓋（只影響這週的檢查），不是永久改時段（那種會直接寫回
        `schedule_entries`、不會出現在這裡），所以目前登記的時段就是「異動前」那個。

        `check_time`／`check_time_start` 存的是公告原文抓出來的時刻（`_TIME_PATTERN`
        是 `\\d{1,2}:\\d{2}`），單位數小時不會補零（例如「1:30」），要先用 `_parse_hhmm`
        正規化成兩位數，否則跟 `old_time` 的 `%02d:%02d` 格式比對永遠對不上，會誤判成
        「有改時間」，畫面也會出現「01:30 更改為 1:30」這種格式不一致的顯示。"""
        if pending["type"] == "skip_check":
            return ""
        raw_new_time = pending.get("check_time") or pending.get("check_time_start")
        if not raw_new_time:
            return ""
        parsed_new = _parse_hhmm(raw_new_time)
        if parsed_new is None:
            return ""
        new_time = f"{parsed_new[0]:02d}:{parsed_new[1]:02d}"
        entry = self._schedule_store.get_entries().get(pending["sn"])
        if entry is None or entry.schedule_hour is None or entry.schedule_minute is None:
            return ""
        old_time = f"{entry.schedule_hour:02d}:{entry.schedule_minute:02d}"
        if old_time == new_time:
            return ""  # 沒有真的改時間（例如只改集數）就不用「原時間 更改為 原時間」
        return f"{old_time} 更改為 {new_time}"
