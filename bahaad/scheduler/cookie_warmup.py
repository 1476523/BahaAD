...
class CookieWarmup:
    ...
    def warm_up(self) -> bool | None:
        ...
        **跳過理由改記 INFO**（使用者 2026-09-11 回報登入態隔了十幾小時無預警失效，
        事後翻日誌完全看不出保活這段時間到底有沒有真的在跑——`_download_active()`／
        `activation_lock` 忙碌兩種跳過原本都只有 DEBUG，DB 日誌只收 INFO 以上，等於
        整段空窗期完全沒有紀錄可查。跳過本身不代表 bug，但「看不出有沒有跳過」會讓
        下次同樣的情況又要從頭排查一輪。"""
        if not self._vault.get_status().get("configured"):
            return None
        if self._download_active():
            logger.info("cookie_warmup：有下載正在進行中，這輪保活跳過")
            return None
        with skip_if_busy(self._activation_lock) as free:
            if not free:
                logger.info("cookie_warmup：下載交握進行中，這輪保活跳過（不輪換登入 cookie）")
                return None
            try:
                return self._cookie_rotation.warm_up_session()
            except Exception:  # noqa: BLE001 - 保活失敗不該讓背景執行緒掛掉
                logger.warning("cookie_warmup 保活瀏覽發生例外（不影響主流程）", exc_info=True)
                return False
    ...
    def check_once(self) -> bool | None:
        ...
        with skip_if_busy(self._activation_lock) as free:
            if not free:
                logger.info("cookie_warmup：下載交握進行中，這輪完整檢查跳過（下輪再查）")
                return None
            try:
                valid = self._cookie_rotation.is_login_valid(sn)
                if not valid:
        ...
        if not valid:
            ...
            # 使用者 2026-09-11：登入態隔了十幾小時無預警失效，翻日誌完全看不出這期間
            # 定期完整檢查到底有沒有跑、跑的時候是不是還正常——「正常」以前完全不記錄，
            # 事後只能看到失效那一瞬間，看不出「什麼時候開始不對勁」。這行是唯一持續存在
            # 的心跳，跟 `cookie_warmup_interval_hours`（預設 6 小時）同週期。
            logger.info("cookie_warmup：定期完整檢查——登入態正常")
        return valid
    ...
    def _run_loop(self) -> None:
        ...
        # `float("-inf")`：確保第一輪一定進 check_once 分支——`time.monotonic()` 在 Windows
        # ＝「開機到現在的秒數」，剛重開機時 `now` 很小，`now - 0.0 >= 6h` 會是 False →
        # 第一次完整登入態檢查被推遲到「開機滿 6 小時」才跑（使用者 2026-09-10：重啟後
        # 沿用同一組 cookie，若它其實已在站方端失效，這 6 小時內只有 warm_up() 的瀏覽、
        # 偵測不到，真的要下載時才踩到「站方維護中」）。比照 `cache/images.py._last_prune`。
        last_full_check = float("-inf")
