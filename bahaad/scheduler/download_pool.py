...
**2026-09-23（使用者回報）**：光靠 `submit()` 觸發同步、加上背景 `_poll_permits` daemon
（每 2 秒對齊一次）還不夠即時——調大上限時，已經 parked 在等 slot 的任務最慢要等 2 秒
才會被放行，使用者在設定頁存檔那一刻感受不到「立即生效」。新增 `sync_now()`，設定頁的
儲存路由（`web/settings.py`）存檔當下就直接呼叫（經 `MainLoop.apply_download_concurrency_
now()`），不用等輪詢。調小依然維持 best effort，不中斷正在跑的下載——「下個要下載時才
套用新上限」這個語意本來就是號誌天生的行為：許可數只在有空閒時才收回，收回之前多出來
的下載繼續跑到完（各自跑完各自的下載冷卻，見 `main_loop._download_worker` 的 finally），
不會被強制中斷或跳過冷卻。
"""
...
class DownloadPool:
    ...
    def sync_now(self) -> None:
        """設定頁改「最大一次下載數」時呼叫（使用者 2026-09-23 回報：調大之後要等
        `_poll_permits` 那條 2 秒一次的背景 daemon 才會反應，不算真的「立即生效」）。
        單靠 `submit()`／`_poll_permits()` 也會慢慢對齊，這支只是讓設定頁儲存那一刻
        就同步一次，不用等最多 2 秒——調小依然是 best effort（見 `_sync_permits()`
        docstring），不中斷正在跑的下載。"""
        self._sync_permits()
