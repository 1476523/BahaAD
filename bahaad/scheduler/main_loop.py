...
**裝置啟用交握**（2026-09-10 補上）：`checklock()`／`unlock()`／`token.php`／`videoStart`
以前只有非 VIP／訪客（`video_src.php` 先回 code 1015）才會透過 `guest_access.clear_for_
download()` 跑到；VIP／已登入直接打 `video_src.php`。使用者實測發現一組 device_id 用一兩
小時後 `video_src.php` 就開始回 **code 1007**（裝置驗證異常），換全新的也一樣，日誌實證
登入態雜湊全程沒變——device_id 沒被站方後端「啟用」是主因。現在 `PlaylistClient.
get_playlist()` 在 `video_src.php` 之前先呼叫 `guest_access.activate_device()` 跑一次
`token.php`→`unlock`→`checklock`→`unlock`→`unlock`→`videoStart`→`token` 確認（比照
aniGamerPlus `Anime.__get_m3u8_dict()`——它每次下載不分 VIP 都跑這段）。非 VIP 交回
呼叫端讓 1015 流程處理。
...
from bahaad.scheduler.download_dir_migration import DownloadDirMigration, MigrationPreview, MigrationStatus
...
class MainLoop:
    def __init__(
        ...
        # 訂閱者 Discord/Telegram 通知（可選依賴，見「真正的推播發送」筆記）——
        # 下載成功時，除了既有的擁有者通知，也順便通知追蹤這部番劇的訂閱者
        ...
        # 更改下載目錄時自動遷移既有檔案（規格見
        # docs/requirements/download_dir_migration.md）——`_downloaded_store`／
        # `_settings`／`_registry` 都已經是這個類別本來就有的依賴，不需要另外注入，
        # 比照 `_downloaded_store` 本身「沒帶就整組功能是 no-op」的可選依賴慣例。
        self._download_dir_migration = (
            DownloadDirMigration(downloaded_store, settings, registry)
            if downloaded_store is not None
            else None
        )
    ...
    def migration_preview(self, old_dir: str, new_dir: str) -> MigrationPreview | None:
        """設定頁存檔前的確認視窗用：有幾筆、大約多少容量需要搬。"""
        if self._download_dir_migration is None:
            return None
        return self._download_dir_migration.preview(old_dir, new_dir)

    def start_migration(self, old_dir: str, new_dir: str) -> bool:
        """使用者在確認視窗按下確認才呼叫。已經有一輪在跑會回傳 `False`（規格：不接受
        疊加/排隊）。"""
        if self._download_dir_migration is None:
            return False
        return self._download_dir_migration.start(old_dir, new_dir)

    def migration_status(self) -> MigrationStatus | None:
        """遷移進度輪詢用。"""
        if self._download_dir_migration is None:
            return None
        return self._download_dir_migration.status()

    def resume_pending_migration(self) -> bool:
        """程式啟動時呼叫一次：上次沒跑完的遷移（程式中途被關掉）接著跑。"""
        if self._download_dir_migration is None:
            return False
        return self._download_dir_migration.resume_if_pending()

    def stop_migration_thread(self) -> None:
        """程式關閉時呼叫，讓遷移背景執行緒盡快結束當前這一輪迴圈（不中斷正在進行的
        單筆複製）。"""
        if self._download_dir_migration is not None:
            self._download_dir_migration.stop()
    ...
    def _download_worker(self, video: VideoInfo, display_name: str, output_dir: Path) -> None:
        ...
        try:
            ...
            # 下載冷卻（round 7 第 10 項 + .0 改進.txt 第 12 項）：這條「下載線」處理完一集
            # 之後、放行下一部之前必須等的秒數，放慢對站方的請求節奏、降低風控（code 1007）。
            # 使用者 2026-09-09 訂正：**不分結果**都冷卻——成功、下載失敗（含重試用盡）、
            # 使用者中止、待搬移，都已經對站方發過一輪請求；freeing 後立刻抓下一部下載才是
            # 1007 連環觸發的主因。只有「程式關閉中」(全域 stop_event) 不睡、要盡快收工。
            # worker slot 在睡眠期間仍被佔著，一批並發各自跑完各自冷卻、滾動放行下一部。
            # registry 記下到期時間，下載列表頁才能顯示「冷卻中，約 N 秒後開始下一部」。
            cooldown = self._settings.get("download_cooldown_seconds", _DEFAULT_DOWNLOAD_COOLDOWN_SECONDS)
            if cooldown > 0 and not self._download_pool.stop_event.is_set():
    ...
    def _notify_download_success(
        ...
        # 這一集自己的封面（動畫瘋 video.php 每集各自一張，不是整部番劇共用那張）——
        # `video` 已經是完整抓過的物件，這裡不用再打一次 API 查，見
        # `bahaad/web/browse.py` 的 `_lookup_episode_info()` 同樣的抓法。管理通知／
        # 訂閱通知（下面）共用同一次計算結果。
        episode_cover_url = next(
            (ep.cover for ep in video.episodes if ep.video_sn == video.video_sn), ""
        ) or None
        canonical_sn, _title = resolve_first_ep_sn(self._anime_cache, video.video_sn)
        anime_cover_url = None
        if self._anime_cache is not None:
            try:
                # 借用同一部番劇其他有快取集數的封面（`canonical_sn` 首集詳細頁可能
                # 從沒被瀏覽過而沒有快取）——見 `get_detail_with_sibling_fallback` 註解，
                # 使用者 2026-09-17 回報部分通知缺番劇封面就是這裡漏了 fallback。
                hit = self._anime_cache.get_detail_with_sibling_fallback(canonical_sn)
            except Exception:  # noqa: BLE001
                hit = None
            anime_cover_url = hit[0].cover_url if hit is not None else None
        ...
        if self._notify_store is not None:
            ...
            send_notification(
                ...
                # 使用者 2026-09-12：跟訂閱通知同樣的「插入 token 附圖」設計，見
                # notify/dispatch.py 的 send_notification()。
                episode_cover=episode_cover_url, anime_cover=anime_cover_url,
            )

        # 訂閱者通知（見「真正的推播發送」筆記）——跟上面擁有者的管理通知完全獨立，
        # `notify_store` 沒設定不影響這裡。sn 要正規化成首集 sn，才會跟訂閱者當初
        # 按「訂閱通知」時存進 `subscriber_follows` 的 sn 一致。
        if self._subscriber_notifier is not None:
            self._subscriber_notifier.notify_episode(
                canonical_sn, display_name, self._notify_episode_label(video),
    ...
    def abort_download(self, video_sn: int) -> bool:
        ...
        **立即**把它從 registry（進行中清單）＋ pending 列拿掉，不等 `_download_worker`
        攔到 `DownloadInterrupted`（使用者 2026-09-09：併發已滿、任務還 parked 在等 slot
        時，之前要等佇列處理到它才會消失，看起來像「取消沒生效」）。已經在跑的下載執行緒
        會在下一次檢查 stop_event 時（片段之間、看廣告等待迴圈）自己拋出中斷、清掉暫存，
        `update_progress()`／`registry.cancel()` 對已經不在 active 的 sn 都是安全的空操作。

        使用者 2026-09-23 回報的真實 bug：手動觸發的下載被 parked 在等併發 slot 時按
        「中止」，`manual_tasks` 那筆持久化紀錄本來要等 `_download_worker` 真的跑到
        `finally`（見 `_download_worker` 尾端）才會清掉——但 parked 任務要等輪到它才會
        真的執行，如果這段空窗期使用者剛好強制重啟（不是等 `_download_worker` 自己跑完
        清乾淨那種正常關閉），`manual_tasks` 那筆舊紀錄就會留著，開機 `resume_manual_
        tasks()` 掃到它、`registry.try_start()` 這時候已經不擋了（active 已經被這裡清
        掉），於是把已經中止的下載重新排回去。修法：跟 `registry`／`pending_store` 一樣，
        這裡也**立即**同步清掉 `manual_tasks` 那筆，不等 worker 自己收尾。"""
        cancelled = self._download_pool.cancel(video_sn)  # 設 per-task 中止旗標
        active = self._registry.is_active(video_sn)
        if not cancelled and not active:
            return False
        self._registry.cancel(video_sn)
        if self._pending_store is not None:
            self._pending_store.remove(video_sn)
        if self._manual_task_store is not None:
            self._manual_task_store.remove_task(video_sn)
        return True
    ...
    def apply_download_concurrency_now(self) -> None:
        """設定頁改「最大一次下載數」儲存當下呼叫（使用者 2026-09-23 回報：調大要
        立即套用到正在下載＋parked 等併發 slot 的任務，不要等 `download_pool.py`
        背景 daemon 那 2 秒一輪的輪詢才反應）。調小維持 best effort，不中斷正在跑
        的下載——見 `download_pool.sync_now()`。"""
        self._download_pool.sync_now()
