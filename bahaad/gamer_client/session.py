...
class GamerSession:
    ...
    def load_cookies(self, cookies: dict[str, str], source: str = "重新登入動畫瘋") -> None:
        """把外部拿到的一組全新 cookie（例如 cookie_rotation.py 從瀏覽器登入流程
        取得的、或保活丟棄式 session 跑完一趟瀏覽動線收集到的）灌進目前這個 session，
        取代舊的，並立刻持久化。跟 __init__ 時的 _load_cookies() 不同：那是唯讀啟動
        載入，這裡是登入態更新後的主動覆寫。`source` 只影響 `_persist_cookies()` 的
        輪換日誌訊息（預設沿用舊行為的「重新登入動畫瘋」），呼叫端可以傳更貼切的說法
        （例如保活丟棄式 session 收集到新 cookie 時）。"""
        with self._request_lock:
            self._session.cookies.clear()
            for name, value in cookies.items():
                self._session.cookies.set(name, value)
            self._persist_cookies(source)

    def build_throwaway_session(self) -> curl_requests.Session:
        """建一個跟目前這個 session 同一套環境偽裝指紋（UA/JA3/Akamai）、但完全獨立
        cookie jar 的 curl_cffi session——保活額外跑一趟模擬瀏覽用（見
        `cookie_rotation.warm_up_session()`），不動到主 session 自己的 cookie 狀態，
        呼叫端確定要採用結果才透過 `load_cookies()` 寫回。跟 `_make_session()` 是同一份
        邏輯（指紋一律讀 IdentityStore 目前值），只是額外公開給外部呼叫。"""
