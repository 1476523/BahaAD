...
class GuestDeviceActivationError(GuestAccessError):
    """交握途中 `token.php` 回 `error.code 1007`（裝置驗證異常）——這組 device_id 已被
    站方標記，`_raise_for_access_error` 已作廢它。呼叫端據此換一組 device_id 重跑。"""
...
class GuestAccessClient:
    ...
    def activate_device(
        ...
        *,
        ...
        """VIP／已登入下載：`video_src.php` 之前先跑一次「裝置啟用」交握，比照
        aniGamerPlus `Anime.__get_m3u8_dict()`——它**每一次下載**（不分 VIP）都先跑
        `token.php` → `unlock`/`checklock` → `videoStart` → `token` 確認才打
        `video_src.php`。BahaAD 以前 VIP 直接打 `video_src.php`，一組 device_id
        用一兩小時後站方就開始回 code 1007，換全新的也一樣（使用者 2026-09-10 拿
        打包版日誌實證：`登入態` 雜湊全程沒變、全新 device_id 也立刻 1007）。

        **呼叫端必須已持有 `activation_lock`**（`PlaylistClient.get_playlist()` 是在
        `with guard:` 裡呼叫這支的）——這裡不自己搶鎖，免得同一把非重入鎖死結。

        回傳 `token.php` 的 info dict（呼叫端看 `vip`／`login`）。非 VIP（或登入已
        失效）→ 只打一次 `token.php` 就交回呼叫端，讓 `video_src.php` 回 1015 後走
        `clear_for_download()` 的看廣告交握（那裡本來就跑完整交握）。code 1007 丟
        `GuestDeviceActivationError`（device_id 已作廢）；地區限制丟
        `GuestGeoBlockedError`；其他 error 丟 `GuestAccessError`。"""
        check = stop_check or (lambda: False)
        note = on_phase or (lambda _text: None)
        prev_label = self._label
        self._label = label or f"video_sn={video_sn}"
        try:
            self._raise_if_stopped(check, video_sn, "裝置啟用交握前")
            note("正在與動畫瘋交握")
            device_id = self._device_manager.get_or_create()
            info = self._gain_access(video_sn, device_id)
            self._raise_for_access_error(info, video_sn)
            if info.get("vip") is not True:
                # 非 VIP／登入失效——交回呼叫端，video_src.php 會回 1015、走
                # clear_for_download() 的看廣告交握。不在這裡多打 unlock/videoStart。
                return info
            self._unlock_dance(video_sn, device_id)
            self._video_start(video_sn)
            # 比照 aniGamerPlus check_no_ad()：VIP 不用等廣告，token 的 `time` 應該
            # 已是 1，這裡只用來推進站方狀態機、順便抓地區限制（缺 `time` 欄）。
            confirm = _as_dict(
                self._session.get(
                    _TOKEN_URL,
            ...
            if "time" not in confirm:
                raise GuestGeoBlockedError(
                    f"{self._label} 遭到動畫瘋地區限制，你的 IP 可能不被動畫瘋認可。"
                )
            logger.info("%s 裝置啟用交握完成（VIP）", self._label)
            return info
        finally:
    ...
    def _raise_for_access_error(self, info: dict, video_sn: int) -> None:
        ...
        if code == "1007":
            ...
            raise GuestDeviceActivationError(
