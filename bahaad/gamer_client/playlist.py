...
class PlaylistClient:
    ...
    def get_playlist(
        ...
        **2026-09-06 改**：`ensure_fresh()` 取代了原本「每次下載都 `invalidate()` 硬換一組
        ...
        is_guest_path = self._guest_identity is not None and not self._is_logged_in()
        with guard:
            if refresh_device_id:
                ...
                if not is_guest_path and guest_access is not None:
                    self._activate_member_device(
                        guest_access, device_manager, video_sn, label,
                        stop_check, on_phase,
                    )
                    # 交握途中碰 1007 會換一組新 device_id——重新取一次再打 video_src.php
    ...
    def _activate_member_device(
        self, guest_access, device_manager, video_sn, label, stop_check, on_phase,
    ):
        """VIP／已登入路徑：`video_src.php` 之前先跑裝置啟用交握（見
        `guest_access.activate_device`）。碰 code 1007（`GuestDeviceActivationError`，
        device_id 已作廢）→ 換一組再跑一次；還是 1007 → 丟 `GamerApiError(1007)`
        （`main_loop` 走「請重新登入動畫瘋」＋整集重試）。純訪客路徑不呼叫這支——
        1015 → `_handshake_then_refetch` → `clear_for_download` 本來就跑完整交握。"""
        if guest_access is None:
            return  # 舊呼叫端／測試沒配置交握 client
        for attempt in (1, 2):
            try:
                guest_access.activate_device(
                    video_sn, label=label, stop_check=stop_check, on_phase=on_phase,
                )
                return
            except GuestDeviceActivationError:
                if attempt == 2:
                    raise GamerApiError(
                        1007, "裝置啟用交握連續兩次都碰 code 1007（裝置驗證異常）"
                    )
                logger.warning(
                    "裝置啟用交握碰 code 1007（%s）——換一組 device_id 再跑一次",
                    _display(label, video_sn),
                )
                device_manager.invalidate()
                device_manager.get_or_create()
            except GuestAccessError as exc:
                # 地區限制／年齡限制／VIP 限定…——包成 GuestHandshakeError 交給 main_loop
                # 既有的下載失敗流程（診斷回報用 GUEST_HANDSHAKE_FAILED）。
