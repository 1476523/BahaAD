...
class GamerServerError(PlaylistError):
    """動畫瘋站方暫時性錯誤——`video_src.php` 回非 200（503 維護中／502…）或回應根本
    不是 JSON（維護頁面是 HTML）。這**不是**帳號/裝置/權限問題，純粹是站方當下不可用，
    過一陣子自己會好。`PlaylistError` 的子類 → `main_loop` 既有的重試流程會照常接手；
    另外 `_download_worker` 會據此排進「延長重試排程」（使用者 2026-09-06：碰到動畫瘋
    503 時當下直接停、沒有重試）。"""
...
class PlaylistClient:
    ...
    def get_playlist(
        ...
        **2026-09-06 改**：`ensure_fresh()` 取代了原本「每次下載都 `invalidate()` 硬換一組
        ...
        with guard:
            ...
            playlist_url = self._fetch_video_src_recover_1007(
    ...
    def _fetch_video_src(
        ...
        if status is not None and status != 200:
            # 503 維護中 / 502 / 500…——站方暫時性錯誤，不是帳號問題，過一陣子自己會好。
        ...
        try:
            ...
            # 維護頁面是 HTML，`.json()` 會拋 JSONDecodeError——包成可重試的站方錯誤，
            # 不要讓它一路衝到「未預期的例外」被判成硬失敗（使用者 2026-09-06）。
            raise GamerServerError(
