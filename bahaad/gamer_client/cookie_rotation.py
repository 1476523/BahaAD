...
from bahaad.gamer_client.session import GamerSession, auth_cookie_digest
...
#
# **2026-09-11 改回 www→ani 兩站、且改用丟棄式 session**：2026-09-08 曾經拿掉 `www`
# 那站、只打 `ani`（見下方歷史說明），但使用者比對 aniGamerPlus 連續一週的日誌
# （`Cookie保護機制請求 偵測到登入憑證即將輪換，已自動完成刷新`）——同一顆帳號、同一個
# 站，aniGamerPlus 從沒斷過，BahaAD 只打 `ani` 之後仍然反覆在同一個 30 分鐘保活週期上
# 觀察到登入態雜湊 A→B→A 這種不像正常單向輪換的來回（兩次保活請求前後相隔 30 分鐘、
# 沒有任何並發，卻拿回舊值），且隔了十幾小時後 session 直接整個過期，跟 aniGamerPlus
# 完全不會發生的狀況對照鮮明。這代表 2026-09-08 那次的診斷（「www/ani 各自發回不同
# BAHARUNE，共用 jar 讓雜湊震盪」）只解釋了原本觀察到的那一種震盪，不是唯一成因，拿掉
# `www` 也沒讓問題消失——真正的差異在於 aniGamerPlus **從不讓保活動到那個長駐、被下載/
# 排程/gossip 共用的 session**，每次都是全新丟棄式 session + 自己收集到的 cookie 走完
# 兩站瀏覽動線，只有真的判斷要刷新（`should_refresh_cookie()`，最多每 90 秒一次）才把
# 結果寫回長駐設定。BahaAD 原本的 `warm_up_session()` 直接用長駐 `GamerSession` 發請求，
# 不管保活的請求本身有沒有踩到什麼問題（CDN 快取回舊回應、伺服器端 session store 還沒
# 同步…），都會透過 `_persist_cookies()` 立刻反映到主 session 上。改成丟棄式 session
# （`GamerSession.build_throwaway_session()`）+ 節流後才用 `load_cookies()` 套用結果，
# 行為對齊實測 7 天零失敗的 aniGamerPlus，也不會再幹擾主 session 進行中的請求。
#
# 舊歷史（2026-09-08）：主 session 分享同一個 cookie jar 時，`www`／`ani` 各自發回不同
# BAHARUNE 曾經讓雜湊在兩值間反覆橫跳、觸發 code 1007；現在保活改在完全獨立的丟棄式
# session 裡走完 `www→ani` 全程再一次性套用結果，不會再有主 session 被兩站中途状態污染
# 的問題。
_WARMUP_URLS = ("https://www.gamer.com.tw/", "https://ani.gamer.com.tw/")

# 節流：偵測到保活拿回不同的登入 cookie 時，最快多久才真的套用回主 session 一次
# （比照 aniGamerPlus `should_refresh_cookie()` 預設 90 秒的節流——避免短時間內某次
# 丟棄式 session 剛好撞上站方回應異常/CDN 快取，一套用就把主 session 搞壞；30 分鐘一輪
# 的保活週期本來就遠低於這個節流門檻，正常情況下不會被卡到）。
_WARMUP_MIN_APPLY_INTERVAL_SECONDS = 90.0
...
class CookieRotation:
    def __init__(
        ...
        *,
        ...
        # 保活節流狀態（見 `_WARMUP_MIN_APPLY_INTERVAL_SECONDS`）——`float("-inf")`
        # 確保第一次保活一定能套用，不用等節流窗口過去（比照 cookie_warmup._run_loop
        # 的 last_full_check 慣例）。
        self._monotonic = monotonic
        self._warmup_apply_lock = threading.Lock()
        self._last_warmup_apply_monotonic = float("-inf")

    def warm_up_session(self) -> bool:
        """跑一趟 `www.gamer.com.tw` → `ani.gamer.com.tw` 的模擬瀏覽動線，讓閒置太久的
        登入 session 保持存活。動畫瘋長時間收不到任何請求時會把 BAHARUNE 判定成過期
        （不是被別的裝置搶走，單純閒置太久），之後只能重新登入。

        **2026-09-11 改用丟棄式 session**（見 `_WARMUP_URLS` 上方的完整說明）：整趟
        `www→ani` 都在 `GamerSession.build_throwaway_session()` 建的獨立 session 上跑，
        不碰長駐 session 的 cookie jar；跑完比對結果有沒有真的跟目前登入態不一樣，
        不一樣才（節流後）透過 `load_cookies()` 套用回長駐 session。

        只有已登入（cookie 有 `BAHAID`）才有意義——遊客沒有 BAHARUNE 可以保溫。回傳有沒有
        真的跑完保溫請求（不代表「有沒有真的套用新 cookie」——沒變化或被節流也算跑完）；
        任何網路例外都吞掉（這只是保溫、不是關鍵路徑，不該讓背景執行緒掛掉）。"""
        current_cookies = self._session.current_cookies()
        if "BAHAID" not in current_cookies:
            return False
        try:
            throwaway = self._session.build_throwaway_session()
            for name, value in current_cookies.items():
                throwaway.cookies.set(name, value)
            headers = {"Accept-Language": "zh-TW,zh;q=0.9,en-US;q=0.8,en;q=0.6"}
            prev: str | None = None
            for url in _WARMUP_URLS:
                if prev:
                    headers = dict(headers, Referer=prev)
                throwaway.get(url, headers=headers, timeout=15)
                prev = url
            harvested = dict(throwaway.cookies)
        except Exception:  # noqa: BLE001 - 保溫失敗不影響任何主流程
            return False

        if auth_cookie_digest(harvested) == auth_cookie_digest(current_cookies):
            return True  # 沒變化，不用套用（多數保活輪次都是這樣，不算失敗）

        with self._warmup_apply_lock:
            now = self._monotonic()
            if now - self._last_warmup_apply_monotonic < _WARMUP_MIN_APPLY_INTERVAL_SECONDS:
                logger.debug("cookie_warmup：偵測到新登入態但節流中，這次不套用（下一輪保活再測一次）")
                return True
            self._last_warmup_apply_monotonic = now
        self._session.load_cookies(harvested, source="保活丟棄式 session")
        return True
