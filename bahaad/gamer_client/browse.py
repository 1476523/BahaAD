...
@dataclass(frozen=True)
class ExpiringLicenseGroup:
    """首頁「即將下架」區塊的一組（使用者 2026-09-23）——站方原始標題如「9月份授權到期
    節目」／「10月上旬授權到期節目」，`label` 照抄原文（含「份」／「上旬」等，不強行
    簡化成單純「N月」，避免混淆全月／上旬這種不同顆粒度的分組）。`items` 的形狀跟
    `SearchResult` 完全一樣（同樣是 `animeRef.php?sn=` 卡片），直接重用同一套解析。"""
...
_EXPIRING_TITLE_RE = re.compile(r"授權到期節目$")


def get_expiring_licenses(http: HttpGetter) -> list[ExpiringLicenseGroup]:
    """首頁「即將下架」區塊（使用者 2026-09-23）：站方把同一個標題（例如「9月份授權到期
    節目」）拆成好幾行 `.animate-theme-list`（每行固定幾張卡），這裡依標題文字原樣合併
    成一組——順序照頁面第一次出現的順序（月份較近的在前）。卡片形狀（`ref_sn`／
    `title`／`cover_url`／`year_text`／`episode_count_text`）跟 `search.php` 的搜尋結果
    完全一樣，直接重用 `_parse_search_result()`。"""
    soup = _fetch_home_soup(http)
    groups: dict[str, list[SearchResult]] = {}
    order: list[str] = []
    for block in soup.select(".animate-theme-list"):
        title_el = block.select_one(".theme-title-block h1.theme-title")
        if title_el is None:
            continue
        label = title_el.get_text(strip=True)
        if not _EXPIRING_TITLE_RE.search(label):
            continue
        if label not in groups:
            groups[label] = []
            order.append(label)
        for a in block.select(".theme-list-block a.theme-list-main"):
            result = _parse_search_result(a)
