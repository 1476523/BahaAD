...
def youranimes_next_season_slug(today: date) -> str:
    """回傳「下一季」的 slug（使用者 2026-09-18：「新番」＝下個季度）——跟
    `youranimes_season_slugs()` 算當季用的同一套規則，往後推 3 個月，不套 14 天
    寬限窗（寬限窗是給「快進入下一季的番劇提早歸類」用的，下一季本身不需要）。"""
    month = ((today.month - 1) // 3) * 3 + 1  # -> 1 / 4 / 7 / 10（當季月份）
    year = today.year
    month += 3
    if month > 12:
        month -= 12
