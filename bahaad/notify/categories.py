"""通知類別的定義／預設範本。規格見 docs/requirements/notify.md「通知類別」
...
`system_new_version`（新版本可用）已於 2026-09-12 移除——上方已經有專門的更新橫幅
UI（見 `bahaad/scheduler/version_check.py` 的 `_on_new_version` 回呼），這則額外的
推播通知純屬重複，使用者要求拿掉；連帶 `@version_tag@`／`@version_body@` 兩個 token
（`notify/tokens.py`）也一併移除。

`CUSTOMIZABLE_CATEGORIES` 少一個 `newanime_full_list`——那則通知內容是「按星期分組」
的固定結構，被使用者範本改壞的風險比讓它可自訂更值得避免。
"""
...
# `newanime_full_list` 不開放自訂範本——內容是「按星期分組＋每組可折疊引用」的固定
# 結構，被使用者範本改壞的風險比讓它可自訂更值得避免（見 new_anime_bulletin.md
# §8c、notify/dispatch.py 的特別處理）。
_NON_CUSTOMIZABLE = {"newanime_full_list"}
CUSTOMIZABLE_CATEGORIES: tuple[str, ...] = tuple(c for c in CATEGORIES if c not in _NON_CUSTOMIZABLE)

# 「管理通知範本」編輯頁版面變寬後改兩欄顯示（使用者 2026-09-12）——這裡明講順序，
# 不是單純把 CUSTOMIZABLE_CATEGORIES 切一半（原本的類別順序跟這兩欄的分組對不上）。
TEMPLATE_COLUMN_LEFT: tuple[str, ...] = (
    "download_success",
    "download_failed",
    "anime_completed",
    "newanime_added",
...
TEMPLATE_COLUMN_RIGHT: tuple[str, ...] = (
    "system_daily_digest",
    "gossip_pause",
    "gossip_delay",
    "gossip_extra_episode",
