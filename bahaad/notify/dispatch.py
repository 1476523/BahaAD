...
def send_notification(
    ...
    **context: object,
    ...
    # 已經組好的安全 Telegram HTML 直接送（不走 render_message 的 token 跳脫路徑，不然
    # `<blockquote>` 會被跳脫成純文字）：
    #   - 新番快訊「完整列表」：`full_list_html`（按星期分組＋每組可折疊引用，見
    #     new_anime_bulletin.md §8c）
    prebuilt = context.get("full_list_html")
    if category == "newanime_full_list" and prebuilt:
        _send_prebuilt_html(store, http, credentials, category, header, str(prebuilt))
        return True

    # 通用版「已經組好的內容直接送」，不綁定特定類別——目前給 gossip_watch.py 的
    # 公告合併通知用（一則公告同時提到好幾部作品時，合併成一則訊息，不套用單一類別
    # 固定的範本格式，見 `_notify_gossip_batch()`）。跟上面 `full_list_html` 的差異只在
    # 不限定 `category == "newanime_full_list"`，機制（Telegram 原樣送、Discord 轉
    # markdown）完全共用 `_send_prebuilt_html()`。
    prebuilt_html = context.get("prebuilt_html")
    if prebuilt_html:
        _send_prebuilt_html(store, http, credentials, category, header, str(prebuilt_html))
        return True

    template = _template_for(store, category)

    # 使用者 2026-09-12：插入 @episode_cover@／@anime_cover@ 就附圖，跟訂閱通知範本
    # 同樣的設計（見 subscriber/dispatch.py 的 notify_episode()）——呼叫端（目前是
    # download_success）算好兩個 URL 傳進 context，這裡依範本實際用了哪個 token
    # 決定要不要附、附哪幾張。**兩個 token 都插入時兩張一起附**（使用者 2026-09-12
    # 定案，推翻先前「集數封面優先、二選一」的設計）——集數封面排前面。token 本身
    # 在訊息文字裡換成空字串，圖片走真正的附圖 API（Discord 多個 embed／Telegram
    # sendMediaGroup），不是把網址貼進內文。
    episode_cover_url = context.pop("episode_cover", None) or None
    anime_cover_url = context.pop("anime_cover", None) or None
    image_urls = []
    if "@episode_cover@" in template and episode_cover_url:
        image_urls.append(episode_cover_url)
    if "@anime_cover@" in template and anime_cover_url and anime_cover_url != episode_cover_url:
        image_urls.append(anime_cover_url)
    context["episode_cover"] = ""
    context["anime_cover"] = ""
    ...
    if credentials.discord_webhook_url:
        ...
        ok, error = senders.send_discord(
            http, credentials.discord_webhook_url, "BahaAD 通知", body, image_urls=image_urls or None
        )
