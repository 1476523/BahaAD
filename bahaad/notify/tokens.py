...
PLACEHOLDER_REFERENCE: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        "下載完成通知適用；有插入才會真的附圖，插入後訊息文字裡看不到網址本身",
    ),
    (
        "@anime_cover@", "整部番劇的封面圖片",
        "下載完成通知適用；跟 @episode_cover@ 都插入時兩張圖一起附上",
    ),
...
# 版面兩欄化用：前幾組排左欄，其餘排右欄（見 placeholder_reference_grouped() 的順序）。
