...
def _home_http(deps):
    """首頁本季新番／週期表要用哪個 HTTP client 爬。

    預設 `browse_http`（無 cookie 訪客 session）——動畫瘋首頁對訪客**不供年齡限制
    （18 禁）番劇**，所以訪客爬回來的清單少了那些項目（使用者 2026-09-10 回報）。
    偵測到動畫瘋登入態存在且未失效時改用帶登入 cookie 的 `browse_member_http`，
    登入帳號（多半已過成年驗證）才看得到完整清單。

    只有這一條路為了「補齊 18 禁」而帶 cookie；其餘網頁爬取仍走無 cookie session
    ...
    try:
        ...
        logger.debug("_home_http 判斷登入態失敗，退回訪客 session", exc_info=True)
    return guest
