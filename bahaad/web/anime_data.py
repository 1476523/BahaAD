...
def _age_aware_browse_http(deps):
    """需要看得到 18 禁番劇完整內容的網頁爬取（首頁本季新番／週期表／番劇詳細頁），
    要用哪個 HTTP client。

    預設 `browse_http`（無 cookie 訪客 session）——動畫瘋對訪客**不供年齡限制
    （18 禁）番劇的完整內容**：首頁清單會漏掉這些番劇（使用者 2026-09-10 回報），
    番劇詳細頁的集數清單也會漏掉這些番劇裡的集數（使用者 2026-09-25 回報：某集
    在首頁「本季新番」正確顯示，但點進詳細頁「本篇」集數按鈕卻沒有那一集）。
    偵測到動畫瘋登入態存在且未失效時改用帶登入 cookie 的 `browse_member_http`，
    登入帳號（多半已過成年驗證）才看得到完整內容。

    只有這幾條路為了「補齊 18 禁」而帶 cookie；其餘網頁爬取仍走無 cookie session
    （見 docs/requirements/anime_cache.md 與 app_shell 的 `browse_http` 註解）。
    """
    guest = getattr(deps, "browse_http", None)
    member = getattr(deps, "browse_member_http", None)
    if member is None or member is guest:
        return guest
    settings = getattr(deps, "settings", None)
    if settings is not None and settings.get("_gamer_login_stale"):
        return guest  # cookie 還在但站方已不認，帶了也是訪客等級、只是白白輪換 BAHARUNE
    try:
        from bahaad.gamer_client.catalog import _session_has_login

        if _session_has_login(member):
            return member
    except Exception:  # noqa: BLE001
        logger.debug("_age_aware_browse_http 判斷登入態失敗，退回訪客 session", exc_info=True)
    return guest
...
def _refetch_and_save_home(deps):
    """實際打網路重抓本季新番＋週期表＋即將下架並存進快取。回傳 (cards, schedule) 或
    None。「即將下架」（`get_expiring_licenses`）跟另外兩個一樣是首頁本身的區塊，
    每次重抓首頁就順便一起抓（使用者 2026-09-23：不要等使用者真的打開「即將下架」
    頁面才查詢，開啟那頁只是顯示已經快取好的結果）。"""
    http = _age_aware_browse_http(deps)
    if http is None:
        return None
    cards = get_newanime(http)
    schedule = get_weekly_schedule(http)
    try:
        expiring = get_expiring_licenses(http)
    except Exception:  # noqa: BLE001 - 「即將下架」是附加區塊，抓失敗不能拖垮整輪重抓
        logger.debug("即將下架區塊爬取失敗", exc_info=True)
        expiring = []
    cache = getattr(deps, "anime_cache", None)
    if cache is not None:
        try:
            cache.save_home(cards, schedule, expiring_groups=expiring)
        except Exception:
            logger.debug("save_home 失敗", exc_info=True)
    _register_images(deps, [c.cover_url for c in cards])
    _register_images(deps, [item.cover_url for group in expiring for item in group.items])
...
def home_data(deps) -> tuple[list[AnimeCard], list[WeeklySchedule], str | None, bool]:
    """回傳 (本季新番, 週期表, browse_error, refreshing)。快取優先——見「觸發 3」的決策。
    ...
    **有可用快取時絕不在這裡等網路**：該重抓就丟背景執行緒，這次先回快取，首頁不會
    ...
    `refreshing`＝True 代表這次回傳的是**已知過期**的快取、背景重抓剛被觸發——呼叫端
    （`web/__init__.py` 的 `index()`）拿這個旗標讓前端知道「等一下再重新整理一次就有
    新資料」，不然使用者（尤其是番劇時段剛過、人卻是這時候才登入/開首頁）會卡在舊
    畫面，`home_auto_refresh.js` 原本只會排「下一個未來時段」的重載、抓不到「剛好
    這次背景重抓已經在跑」這個情況（使用者 2026-09-12 回報：番劇更新時間到、也下載
    完了，登入後首頁還是舊的，要手動重新整理才會出現）。
    """
    cache = getattr(deps, "anime_cache", None)
    http = _age_aware_browse_http(deps)
    ...
    if cached is not None:
        ...
        if cache is not None and home_refresh_decision(
            ...
            refreshing = True
        return cards, schedule, None, refreshing
    ...
    try:
        expiring = get_expiring_licenses(http)
    except Exception:  # noqa: BLE001 - 「即將下架」是附加區塊，抓失敗不影響首頁本身
        logger.debug("即將下架區塊爬取失敗", exc_info=True)
        expiring = []
    ...
    _register_images(deps, [c.cover_url for c in cards])
    _register_images(deps, [item.cover_url for group in expiring for item in group.items])
    return cards, schedule, None, False


def expiring_licenses_data(deps) -> list[ExpiringLicenseGroup]:
    """「即將下架」頁用——純讀快取，不打網路（使用者 2026-09-23：這頁「開啟是最後一次
    確認」，資料一律跟著首頁本身的抓取節奏更新，見 `_refetch_and_save_home()`／
    `home_data()`）。沒有快取（`anime_cache` 未啟用，或從沒抓過首頁）就回空清單。"""
    cache = getattr(deps, "anime_cache", None)
    if cache is None:
        return []
    try:
        groups = cache.get_expiring_groups()
    except Exception:  # noqa: BLE001 - 純讀快取失敗不該讓這頁掛掉
        logger.debug("讀取即將下架快取失敗", exc_info=True)
        return []
    _register_images(deps, [item.cover_url for group in groups for item in group.items])
    return groups
...
def anime_detail_data(
    ...
    if cache is not None:
        ...
        if hit is None and cache_only:
            ...
            hit = cache.get_detail_with_sibling_fallback(video_sn)
        if hit is not None:
...
def anime_cards_for_sns(
    deps, items: list[tuple[int, str | None]], *, ttl_days: int, cache_only: bool
) -> list[dict]:
    """把一串 `(sn, fallback_title)` 轉成訂閱列表卡片用的 dict list（`video_sn`／`title`／
    `raw_title`／`cover_url`／`episode_count`／`no_cache`）。訂閱列表頁（擁有者的排程
    訂閱）與訂閱者「我的訂閱」共用這套組法，確保兩邊卡片顯示（封面／集數）一致
    （使用者 2026-09-12：「番劇的顯示需與訂閱列表顯示相同」）。"""
    cards: list[dict] = []
    for sn, fallback_title in items:
        detail, _err = anime_detail_data(deps, sn, ttl_days=ttl_days, cache_only=cache_only)
        if detail is None:
            title = fallback_title or f"sn {sn}"
            cards.append(
                {
                    "video_sn": sn,
                    "title": title,
