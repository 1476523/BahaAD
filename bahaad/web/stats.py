...
from bahaad.stats.reporter import _DEFAULT_LOOP_SECONDS
...
@stats_bp.route("/ui/stats/status")
def status():
    """排行榜頁「即時更新」文字前的呼吸燈用——伺服器連線狀態／下次重試秒數／本機
    還沒送出的資料筆數（使用者 2026-09-13：不要只在本機獨立顯示，要看得出跟伺服器
    的同步狀態）。`connected` 是「回報有沒有失敗中」，不是「剛剛這個請求成不成功」
    ——沒有裝 stats_pending_store 就當作沒有這個功能，回中性值讓前端顯示灰燈。"""
    pending = getattr(_deps(), "stats_pending_store", None)
    if pending is None:
        return jsonify({"connected": None, "pending_count": 0, "next_retry_seconds": None,
                         "check_interval_seconds": _DEFAULT_LOOP_SECONDS})
    snap = pending.snapshot()
    pending_count = len(snap.get("counters") or {}) + len(snap.get("anime") or [])
    attempt, _phase, next_at = pending.get_retry_state()
    now = time.time()
    next_retry_seconds = int(next_at - now) if next_at and next_at > now else None
    return jsonify({
        "connected": attempt == 0,
