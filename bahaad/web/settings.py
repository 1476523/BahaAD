...
_FIELDS: list[_SettingField] = [
    ...
    _SettingField(
        ...
        frontend_min=10,
        help_text="每處理完一集（不論成功、失敗或中止）之後、這條下載線放行下一部之前的等待秒數，"
        "放慢對站方的請求節奏、降低風控（code 1007）機率。最少 10 秒。",
    ),
    _SettingField(
...
def index():
    ...
    if request.method == "POST":
        partial = {}
        port_changed = False
        old_download_dir = deps.settings.get("download_dir", _DEFAULT_DOWNLOAD_DIR)
        try:
        ...
        # 使用者 2026-09-23：調大「最大一次下載數」要立即生效，不要等背景 daemon
        # 那 2 秒一輪的輪詢才反應
        if "max_concurrent_downloads" in partial and deps.main_loop is not None:
            deps.main_loop.apply_download_concurrency_now()
        migration_extra = {}
        # 更改下載目錄時自動遷移既有檔案（docs/requirements/download_dir_migration.md）：
        # 路徑本身**一律立刻儲存**（新任務直接寫新目錄，跟原本行為一致）；只有「要不要
        # 搬既有檔案過去」需要使用者另外確認——這裡只回傳掃描結果，不觸發任何複製，
        # 真的開始遷移要靠使用者在前端確認後另外呼叫 `/settings/download-dir-migrate/start`。
        if "download_dir" in partial and deps.main_loop is not None:
            preview = deps.main_loop.migration_preview(old_download_dir, partial["download_dir"])
            if preview is not None and preview.count > 0:
                migration_extra = {
                    "needs_migration_confirm": True,
                    "migration_old_dir": old_download_dir,
                    "migration_new_dir": partial["download_dir"],
                    "migration_count": preview.count,
                    "migration_total_size_bytes": preview.total_size_bytes,
                }
        if port_changed:
            return _port_change_response(deps, message)
        return form_result(message, endpoint="settings.index", json_extra=migration_extra or None)
    ...
    return render_template(
        ...
        migration_status=_migration_status_json(deps),
    )


def _failed_items_json(deps, video_sns: list[int]) -> list[dict]:
    """失敗清單附上番劇名稱，前端「重新下載」按鈕才看得懂列出來的是哪一部——單純
    `video_sn` 數字對使用者沒有意義。查不到標題（理論上不該發生，紀錄還在資料庫裡
    才會被列進失敗清單）就退回顯示 sn 本身。"""
    items = []
    for sn in video_sns:
        title = deps.main_loop.downloaded_episode_anime_title(sn) if deps.main_loop is not None else None
        items.append({"video_sn": sn, "title": title or f"sn={sn}"})
    return items


def _migration_status_json(deps) -> dict | None:
    """`MigrationStatus`（dataclass）轉成前端要的 JSON 結構，或沒有進行中的遷移時回
    `None`——`deps.main_loop` 沒有時（測試／未組裝）也回 `None`。設定頁首次載入（含
    重新整理）跟輪詢端點共用同一個轉換，確保欄位一致。"""
    if deps.main_loop is None:
        return None
    status = deps.main_loop.migration_status()
    ...
    return {
        ...
        "failed": _failed_items_json(deps, status.failed_video_sns),
    }


@settings_bp.route("/settings/download-dir-migrate/start", methods=["POST"])
def download_dir_migrate_start():
    """使用者在確認視窗按下確認才呼叫——`download_dir` 本身在 `index()` POST 當下就已
    經存檔了，這裡單純觸發背景遷移既有檔案。"""
    deps = current_app.config["DEPS"]
    if deps.main_loop is None:
        return jsonify({"ok": False, "error": "目前無法遷移"}), 409
    old_dir = (request.form.get("old_dir") or "").strip()
    new_dir = (request.form.get("new_dir") or "").strip()
    if not old_dir or not new_dir:
        return jsonify({"ok": False, "error": "缺少來源/目的目錄"}), 400
    started = deps.main_loop.start_migration(old_dir, new_dir)
    if not started:
        return jsonify({"ok": False, "error": "已經有一輪遷移在背景進行中"}), 409
    return jsonify({"ok": True})


@settings_bp.route("/settings/download-dir-migrate/status")
def download_dir_migrate_status():
    """遷移進度輪詢——前端每隔幾秒問一次，直到 `finished` 才收掉進度畫面。"""
    deps = current_app.config["DEPS"]
    if deps.main_loop is None:
...
@settings_bp.route("/settings/public-domain", methods=["POST"])
def set_public_domain():
    """公開模式的「域名設定」（docs/requirements/subscriber_notify.md）：公開分享
    連結、以及訂閱通知裡 `https://域名/anime/sn` 連結的基底，同時也是訂閱者中繼
    綁定用的 callback 網址來源。只接受 `https://` 開頭的網域根，不接受路徑／查詢
    字串——比照 access_gate_server 那邊 `subscriber_relay.is_valid_callback_base_url`
    的同一組格式要求，兩邊各自驗證一次，不共用 import（見該模組說明的隔離界線）。"""
    deps = current_app.config["DEPS"]
    domain = (request.form.get("public_domain") or "").strip().rstrip("/")
    if domain and not re.match(r"^https://[^\s/?#]+(?::\d+)?$", domain):
        return form_result(
            "域名格式不正確，需為 https:// 開頭的網域根網址（不含路徑／查詢字串）",
            endpoint="settings.index", ok=False,
        )
    deps.settings.update({"public_domain": domain})
    return form_result("已更新域名設定", endpoint="settings.index")
...
def save_field(key: str):
    ...
    if key == "download_dir" and deps.main_loop is not None:
        ...
        if moved:
            message += f"；已把 {moved} 集待搬移的下載搬進新的下載目錄"
    if key == "max_concurrent_downloads" and changed and deps.main_loop is not None:
        deps.main_loop.apply_download_concurrency_now()
    if key == "web_port" and changed:
        return _port_change_response(deps, message)
    migration_extra = {}
    # 每欄獨立「儲存」按鈕跟底部「全部儲存」共用同一套確認流程（見 index() 的完整說明）
    if key == "download_dir" and changed and deps.main_loop is not None:
        preview = deps.main_loop.migration_preview(old_download_dir, value)
        if preview is not None and preview.count > 0:
            migration_extra = {
                "needs_migration_confirm": True,
                "migration_old_dir": old_download_dir,
                "migration_new_dir": value,
                "migration_count": preview.count,
                "migration_total_size_bytes": preview.total_size_bytes,
            }
    return form_result(message, endpoint="settings.index", json_extra=migration_extra or None)
