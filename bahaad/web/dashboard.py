...
def update_pending():
    """目前的更新狀態——給 base.html 的 `static/update_nag.js` 輪詢用（重點更新鎖定時
    這個端點在豁免名單裡，讓被鎖在首頁的 JS 也問得到）：

    - 差異檔已下載好可套用 → 直接回 `_pending_update`（含 `policy` 欄位）。
    - 還沒下載好、但 GitHub 上偵測到新版（`_update_available`）→ 回
      `{"policy": null, "available": {...}}`，讓開著的分頁即時長出「背景下載中」橫幅
      （使用者 2026-09-09：從系統匣按「檢查更新」後，開著的網頁也該顯示，不用換頁）。
    - 都沒有 → `{"policy": null}`。
    """
    deps = current_app.config["DEPS"]
    pending = deps.settings.get("_pending_update")
    if pending:
        return jsonify(pending)
    return jsonify({"policy": None, "available": deps.settings.get("_update_available")})
...
def apply_update():
    ...
    「正在重新啟動」遮罩、輪詢 `/update/version`，等伺服器回來就把**目前這個分頁**
    重新載入（使用者 2026-09-09：手動開的分頁關不掉，改成重新整理）。沒有 JS 時退回
    傳統表單送出 → `redirect` 回首頁（伺服器隨即關閉，畫面會短暫斷線，屬預期），
    這種情形才靠 `_reopen_web_on_start` 在重啟後另開分頁。"""
    deps = current_app.config["DEPS"]
    ...
    # 無 JS 退路：伺服器關掉後這個分頁就停在斷線的畫面、沒辦法自己重新整理，
    # 靠重啟後的行程另開一個分頁（沿用改連接埠重啟的同一個旗標）。
