...
def _subsystem_media_override(deps, image_hash: str) -> tuple[bytes, str] | None:
    """選擇性子系統的素材刻意走跟真正番劇封面同一支端點——這裡先問一次是不是這個
    雜湊，是的話直接回對應位元組，不繼續往下走真正封面那條路。子系統不存在（公開
    建置）時原樣回 None，呼叫端照常走原本的封面快取邏輯。"""
...
def image(url_hash: str):  # noqa: A002 - route 參數就叫這個名字最直白
    ...
    override_hit = _subsystem_media_override(deps, url_hash)
    if override_hit is not None:
        data, content_type = override_hit
...
def image_batch():
    ...
    if store is None or fetcher is None:
        # 選擇性子系統的素材（見 `_subsystem_media_override()`）不需要
        # anime_cache／image_fetcher，但這兩個沒開的話真正的封面本來就抓不到，
        # 索性整批回空，跟原本行為一致。
        return jsonify({}), 200
