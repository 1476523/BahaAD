...
# `/cache/img-batch` 一次回應的位元組上限——超過就先回這些、其餘標記 pending，
# 前端下一批再要。避免單一回應變成十幾 MB。
_BATCH_BYTE_BUDGET = 3 * 1024 * 1024
...
def _ad_campaign_media(deps, image_hash: str) -> tuple[bytes, str] | None:
    """推廣卡片的圖片刻意走跟真正番劇封面同一支端點（使用者 2026-09-20，見
    `bahaad/ad_promo/campaigns.py` `cache_hash_for()` 開頭說明）——這裡先問一次
    `ad_campaign_poller` 這個雜湊是不是廣告，是的話直接回廣告位元組，不繼續往下
    走真正封面那條路。`ad_campaign_poller` 不存在（公開建置沒有 ad_promo 套件）
    時原樣回 None，呼叫端照常走原本的封面快取邏輯。"""
    poller = getattr(deps, "ad_campaign_poller", None)
    if poller is None:
        return None
    return poller.media_for_cache_hash(image_hash)


@cache_bp.route("/cache/img/<url_hash>")
def image(url_hash: str):  # noqa: A002 - route 參數就叫這個名字最直白
    deps = current_app.config["DEPS"]
    ad_hit = _ad_campaign_media(deps, url_hash)
    if ad_hit is not None:
        data, content_type = ad_hit
        resp = Response(data, mimetype=content_type)
        resp.headers["Cache-Control"] = f"public, max-age={_MAX_AGE_SECONDS}"
        return resp
...
@cache_bp.route("/cache/img-batch", methods=["POST"])
def image_batch():
    """一次要一整頁的封面——回 JSON `{"<hash>": "data:image/...;base64,..." 或 null}`。

    本機有過濾／代理軟體（AdGuard for Windows 之類）時，一頁 ~60 張封面各自對
    127.0.0.1 開連線的突發會被重設（使用者 2026-09-09 `net::ERR_CONNECTION_RESET`）。
    前端 `img_loader.js` 改成打這一支：**一個請求**拿回一批封面的 bytes（base64），
    連線數從 ~60 降到 2-3。

    只回**磁碟上已經有**的（讀檔快、不阻塞）；還沒抓的排進背景佇列、回 `null`
    （前端顯示佔位、過幾秒再要一批）。單一回應 bytes 超過 `_BATCH_BYTE_BUDGET`
    就先回這些、其餘也標 `null`。"""
    deps = current_app.config["DEPS"]
    store = getattr(deps, "anime_cache", None)
    fetcher = getattr(deps, "image_fetcher", None)
    poller = getattr(deps, "ad_campaign_poller", None)
    if store is None or fetcher is None:
        # 廣告位元組（見 `_ad_campaign_media()`）不需要 anime_cache／image_fetcher，
        # 但這兩個沒開的話真正的封面本來就抓不到，索性整批回空，跟原本行為一致。
        return jsonify({}), 200

    payload = request.get_json(silent=True) or {}
    hashes = payload.get("hashes")
    if not isinstance(hashes, list):
        return jsonify({"error": "hashes 必須是陣列"}), 400
    hashes = [str(h) for h in hashes[:200]]

    out: dict[str, str | None] = {}
    budget = _BATCH_BYTE_BUDGET
    for h in hashes:
        if h in out:
            continue
        ad_hit = poller.media_for_cache_hash(h) if poller is not None else None
        if ad_hit is not None:
            ad_data, ad_content_type = ad_hit
            if budget <= 0:
                out[h] = None  # 這批塞不下了，前端下一輪再要
                continue
            budget -= len(ad_data)
            out[h] = "data:%s;base64,%s" % (ad_content_type, base64.b64encode(ad_data).decode("ascii"))
            continue
        rec = store.image_record(h)
        url = rec["url"] if rec else None
        path = None
        if url and fetcher.has_local(url):
            path = fetcher.path_for(url)
        elif rec is None:
            guessed = fetcher.path_for_hash(h)
            path = guessed if guessed.is_file() else None
        ...
        try:
            ...
            continue
        budget -= len(raw)
        ctype = (rec.get("content_type") if rec else None) or "image/jpeg"
        out[h] = "data:%s;base64,%s" % (ctype, base64.b64encode(raw).decode("ascii"))
        if rec and not rec["fetched_at"] and url:
            try:
                store.record_image(url, rec.get("content_type"), len(raw))
            except Exception:  # noqa: BLE001
                logger.debug("回填 fetched_at 失敗：%s", url, exc_info=True)
