...
def _gossip_items(deps) -> list[dict]:
    ...
    for event in events:
        ...
        sn = event["sn"]
        # 使用者更名優先（使用者 2026-09-03：通知要用改過的名字，不是原始標題），
        # 再來是番劇快取的完整標題。使用者 2026-09-23：標題（卡片標頭、可點連結）
        # 一律要顯示完整番劇名稱，不要用公告原文裡可能簡稱/縮寫過的名字
        # （`title_in_gossip` 只是拿來比對用的、公告文字裡實際寫的那個名字）——
        # 原始公告文字本身（`raw_clause`，引用框那段）維持不動，不受這裡影響。
        # `title_in_gossip` 因此退到最後一個 fallback，只有連快取都查不到（例如
        # 還沒抓過這部番劇的詳細頁）才會用它頂著顯示，總比顯示 sn 數字好。
        title = (
            renames.get(sn)
            or _cache_title(sn)
