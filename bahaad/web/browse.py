...
from bahaad.store.schedule_list import _tag_before, _upsert_entry, parse_text, render_text
...
def _subscribe_video(deps, video_sn: int) -> dict:
    ...
    subscribe_sn(deps.schedule_store, video_sn, weekday, hour, minute)
...
def _anime_already_subscribed(deps, video_sn: int) -> bool:
    ...
    if not subscribed or video_sn in subscribed:
        return video_sn in subscribed
    try:
        gk = cache.group_key_for(video_sn)
        if gk is not None and cache.group_members(gk) & subscribed:
            return True
        title = cache.anime_title_for(video_sn)
        if title:
            title = title.strip()
            return any(cache.anime_title_for(sn) == title for sn in subscribed)
    except Exception:  # noqa: BLE001
        return False
    return False
...
def unsubscribe(video_sn: int):
    ...
    cleaned = None
    if unsubscribe_sn(deps.schedule_store, video_sn)["removed"]:
    ...
    _stats_subscription(deps, video_sn, subscribed=False)
...
def rename_anime(video_sn: int):
    ...
    idx = next(
        (i for i, line in enumerate(lines) if line.line_type == "entry" and line.sn == video_sn), None
    )
    ...
    lines = _upsert_entry(
        lines, video_sn, tag, line.mode, name,
...
def subscriptions():
    ...
    for entry in sorted(entries, key=lambda e: e.sn):
