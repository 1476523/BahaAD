...
def anime_detail(video_sn: int):
    ...
    # 播放前推廣圖要顯示的內容——渲染這頁當下就先抽好一個，JS 只負責「什麼時候
    # 顯示」（見 ad_promo_player.js maybeSchedulePromo()）。這個功能整包在
    # `bahaad.ad_promo`（不進公開 repo），選擇性 import，套件不存在時 promo_pick
    # 就是 None，_player_overlay.html 本來也不會被渲染出來（同一個套件）。
    try:
        from bahaad.ad_promo.routes import player_promo_pick

        promo_pick = player_promo_pick(getattr(deps, "ad_campaign_poller", None))
    except ImportError:
        promo_pick = None
...
try:
    from bahaad.playback_guard.checks import looks_like_browser_media_request as _looks_like_browser_media_request
except ImportError:
    def _looks_like_browser_media_request() -> bool:
        # `bahaad.playback_guard` 沒隨這份原始碼／建置附上（公開 repo 不含這個套件，
        # 見 `scripts/publish_public.py` 開頭說明）——退回一律放行，不影響公開原始碼
        # 建置的本機播放，只是少了這層額外的標頭檢查。
        return True
...
def play_episode(video_sn: int):
    ...
    使用者 2026-09-20：加一層 `_looks_like_browser_media_request()` 檢查，擋掉 IDM
    這類偵測到影音連結就直接複製網址下載的工具（回報親測 IDM 能直接把 m3u8 整集
    抓下來，繞過播放器、也就完全繞過播放前的推廣圖）。不是滴水不漏的防護，只是
    拉高一般下載管理器的門檻，見該函式開頭說明。"""
    deps = current_app.config["DEPS"]
...
def subscriptions():
    ...
    # 推廣卡片（使用者 2026-09-17／2026-09-18）——只在公開模式（訪客／會員）看得到，
    # 隨機插在卡片格裡的某個位置，不放進 `subscribed` 這份真正的訂閱清單，「共 N
    # 部動畫」計數才不會被多算一筆。這個功能整包在 `bahaad.ad_promo`（不進公開
    # repo，見該套件說明），這裡選擇性 import，套件不存在時就完全不顯示推廣卡片，
    # 不影響這頁其他功能。
    try:
        from bahaad.ad_promo.routes import promo_card_slot

        promo_ctx = promo_card_slot(cache_only, len(subscribed), poller=getattr(deps, "ad_campaign_poller", None))
    except ImportError:
    ...
    # 使用者 2026-09-17 回報：推廣卡片的隨機位置在單純「重新整理」時沒有變化，切到
    # 別頁再回來才會變——瀏覽器沒有明講的捷徑重新整理（沒有 no-store）下，對沒有
    # Cache-Control／驗證標頭的一般回應會用試探性快取判斷要不要真的重打伺服器。
    # 這頁本來就是每個人看到的內容都不一樣（訂閱清單、推廣卡片位置與 nonce），
    # 不該被瀏覽器快取，明講 no-store 讓「重新整理」保證每次都是全新的伺服器回應。
