...
from flask import (
    Blueprint, abort, current_app, flash, g, jsonify, make_response, redirect, render_template,
...
from bahaad.stats.collector import resolve_first_ep_sn, subscription_count
...
_THEMES = {"light", "dark"}
# 排版模式手動覆寫（使用者 2026-09-21）——"auto" 是預設（維持原本純 CSS
# `(pointer: coarse)` 自動判斷手機的行為）；智慧電視等裝置可能被誤判成觸控裝置，
# 讓使用者自己手動選「電視」/「電腦」蓋掉自動判斷，或反過來手動選「手機」。見
# style.css `[data-layout-active]` 開頭說明、shell.js `BahaADLayout`。
_LAYOUT_MODES = {"auto", "mobile", "tv", "desktop"}
...
def _canonical_detail_sn(deps, video_sn: int) -> int | None:
    """`video_sn` 如果不是它所屬番劇目前的訂閱項目所在的 sn，回傳那個 sn（呼叫端
    據此導向）；沒有訂閱、或查無群組／標題資訊，回 None，維持原樣渲染。

    使用者 2026-09-20：動畫瘋同一部番劇偶爾會拆成好幾個不同 video_sn 的目錄項目
    （例如兩個 sn 都是《MAO 摩緒》，各自的集數清單、評價人數卻不一樣——是站方
    兩筆獨立的目錄項目，不是同一筆資料的不同集數），各自都能被卡片連過去、各自
    建立獨立的番劇頁，沒有導向的話同一部番劇看起來像好幾部。只在**已經訂閱**的
    情況下才導向（`_subscribed_sibling_sn`——先查 episode_group，查不到再退回
    比對番劇標題字串，兩個獨立目錄項目常常沒有共同的集數清單、只能靠標題比對接
    起來），沒訂閱過的番劇完全不導向，維持原本「借用同一部番劇其他有快取集數」
    的既有行為（見 `anime_detail_data()`／`get_detail_with_sibling_fallback()`）。
    單純下載過（沒訂閱）也不會導向——這是使用者明確要求的優先順序：下載不算數，
    訂閱／收藏才算數。刻意不用 `group_key_for()` 當退回依據：那只是「同一頁
    集數清單裡最小的 sn」，同一部番劇正常的不同集數（例如第 2、3 集）本來就會
    落在同一個 group 裡，把它們也導向掉會壞掉既有的訂閱鈴鐺／借用快取邏輯。"""
    subscribed = _subscribed_sibling_sn(deps, video_sn)
    if subscribed is not None and subscribed != video_sn:
        return subscribed
    return None
...
def anime_detail(video_sn: int):
    ...
    canonical_sn = _canonical_detail_sn(deps, video_sn)
    if canonical_sn is not None:
        return redirect(url_for("browse.anime_detail", video_sn=canonical_sn, **request.args))
    ...
    # 選擇性子系統掛勾——套件不存在時 promo_pick 就是 None。
    try:
        promo_pick = import_module("bahaad.ad_promo.routes").player_promo_pick(
            getattr(deps, "ad_campaign_poller", None)
        )
    except ImportError:
        promo_pick = None
    ...
    return render_template(
        ...
        #
        # 這個旗標決定要不要在頁面上顯示遠端播放鈕——2026-09-20 把 HLS 的播放
        # token／manifest／片段這幾支**路由本身**（不只是防護邏輯）都搬進了
        # `playback_guard/routes.py`（選擇性子系統，公開原始碼建置時不存在）。
        # `hls_cache` 只是底層轉檔／快取工具，本身仍是公開的，單靠它判斷會導致
        # 公開原始碼建置出來的版本：按鈕還在，點下去卻是 404（真正的路由沒被
        # 註冊）。加上 `_PLAYBACK_GUARD_AVAILABLE` 一起判斷，公開建置在這個情境
        # 下就乾淨不顯示按鈕，跟其他選擇性子系統的降級方式一致。
        remote_playback=(
            (public or _is_remote_request())
            and getattr(deps, "hls_cache", None) is not None
            and _PLAYBACK_GUARD_AVAILABLE
        ),
    )
...
try:
    _looks_like_browser_media_request = import_module("bahaad.playback_guard.checks").looks_like_browser_media_request
    _PLAYBACK_GUARD_AVAILABLE = True
except ImportError:
    def _looks_like_browser_media_request() -> bool:
        # 選擇性子系統沒隨這份原始碼／建置附上——退回一律放行，不影響本機播放，
        # 只是少了這層額外的標頭檢查。
        return True

    _PLAYBACK_GUARD_AVAILABLE = False
...
def play_episode(video_sn: int):
    ...
    使用者 2026-09-20：加一層 `_looks_like_browser_media_request()` 檢查，降低被
    獨立下載工具直接複製網址繞過播放器抓取的機率。不是滴水不漏的防護，只是拉高
    門檻，見該函式開頭說明。"""
    deps = current_app.config["DEPS"]
    if _public_episode_blocked(deps, video_sn) or not _looks_like_browser_media_request():
        abort(404)
...
_EXPIRING_LABEL_SUFFIX_RE = re.compile(r"授權到期節目$")


@browse_bp.route("/browse/expiring")
def expiring():
    """即將下架——首頁「N月份／N月上旬授權到期節目」區塊（使用者 2026-09-23）。純讀
    快取（`expiring_licenses_data()`），資料跟著首頁本身的抓取節奏一起更新，開啟這頁
    只是顯示已經快取好的結果，不會在這裡另外打網路，所以不用像近期熱播／最新上架
    那樣先出殼再非同步拿內容。不給收藏鈴鐺——這些作品要下架了，不是新開播，訂閱
    等未來集數通知沒有意義。"""
    from bahaad.web.anime_data import expiring_licenses_data

    deps = current_app.config["DEPS"]
    groups = expiring_licenses_data(deps)
    # 使用者鍵名刻意不叫 "items"——dict 本身就有 .items() 方法，Jinja 點記法
    # `column.items` 在模板裡會撞到那支內建方法，不是我們塞的那份清單（真的踩到
    # 這個雷才發現，見 expiring.html 的 `column.cards`）。
    columns = [
        {
            "label": _EXPIRING_LABEL_SUFFIX_RE.sub("", group.label),
            "cards": _section_items(deps, group.items),
        }
        for group in groups
    ]
    return render_template("expiring.html", columns=columns)
...
def _subscribe_video(deps, video_sn: int) -> dict:
    ...
    _stats_subscription(deps, video_sn)
    return {"subscribed": True}


def _stats_subscription(deps, video_sn: int) -> None:
    """即時匿名使用統計——訂閱／退訂用該番劇首集 sn 當 key（使用者 2026-09-08）。
    人數當場用 `subscription_count()` 重新算（擁有者排程訂閱 + 各訂閱者身分追蹤數
    相加，使用者 2026-09-16），呼叫端只需要在異動排程清單之後呼叫這裡一次——不管
    是訂閱還是退訂，都是同一份「重新算完整總數」的邏輯，不會因為只看自己這次的
    動作把訂閱者那邊已經算進去的人數蓋掉。"""
    ...
    try:
        ...
        collector.record_subscription(first_sn, title, count=subscription_count(deps, first_sn))
...
def unsubscribe(video_sn: int):
    ...
    _stats_subscription(deps, entry_sn)
...
def subscriptions():
    ...
    ttl_days = int(deps.settings.get("anime_cache_ttl_days", _DEFAULT_ANIME_CACHE_TTL_DAYS))
    subscribed = []
    # 公開模式專用的週期表側板（使用者 2026-09-16：管理模式已經有「排程清單」側板可以
    # 編輯，公開模式訪客沒有登入入口進不去那頁——改給他們一個唯讀的週期表，直接借
    # 這頁本來就會渲染的訂閱項目分天顯示，不重新打站方、也不跟首頁那份「本季新番全部
    # 番劇」的週期表混在一起，只放這份訂閱列表裡本來就有的項目）。
    ...
    for entry in sorted(entries, key=schedule_sort_key):
        ...
        if detail is None:
            ...
            if cache_only:
                ...
                _append_weekly_entry(by_weekday, entry, item, episode_text="尚無快取")
            continue
        ...
        subscribed.append(item)
        if cache_only:
            _append_weekly_entry(by_weekday, entry, item, episode_text=f"共 {episode_count} 集")
    public_weekly_schedule = (
        [{"day": day, "entries": by_weekday.get(weekday, [])} for weekday, day in enumerate(WEEKDAY_ORDER, start=1)]
        if cache_only
        else None
    )
    if cache_only:
        subscribed = subscribed + _extra_downloaded_for_public(deps, {entry.sn for entry in entries})
    # 選擇性子系統掛勾——套件不存在時 promo_ctx 是空值，不影響這頁其他功能。
    try:
        promo_ctx = import_module("bahaad.ad_promo.routes").promo_card_slot(
            cache_only, len(subscribed), poller=getattr(deps, "ad_campaign_poller", None)
        )
    except ImportError:
        promo_ctx = {"promo_slots": {}}
    response = make_response(render_template(
        "subscriptions.html",
        subscribed=subscribed,
        public_weekly_schedule=public_weekly_schedule,
        **promo_ctx,
        **_public_card_ctx(deps),
    ))
    # 這頁本來就是每個人看到的內容都不一樣，不該被瀏覽器快取，明講 no-store 讓
    # 「重新整理」保證每次都是全新的伺服器回應（使用者 2026-09-17 回報過瀏覽器
    # 試探性快取導致單純重新整理沒有拿到新回應的問題）。
    response.headers["Cache-Control"] = "no-store"
    return response


def _append_weekly_entry(by_weekday: dict[int, list[dict]], entry, item: dict, *, episode_text: str) -> None:
    by_weekday.setdefault(entry.schedule_weekday, []).append(
        {
            "video_sn": item["video_sn"],
            "title": item["title"],
            "time_text": f"{entry.schedule_hour or 0:02d}:{entry.schedule_minute or 0:02d}",
            "episode_text": episode_text,
        }
    )


def _extra_downloaded_for_public(deps, schedule_sns: set) -> list[dict]:
    """公開模式的「番劇列表」頁，除了擁有者真正的訂閱項目，還要能看到已經下載完成
    但沒有排進排程訂閱的番劇（使用者 2026-09-18：公開模式已經拿掉下載列表頁
    ——訪客／會員只剩這頁看得到，已下載卻不在訂閱清單裡的番劇會完全找不到、
    點不進去，是很大的體驗問題）。

    用 episode_group 展開比對是不是「已經算進訂閱清單」，不是直接比 sn 是否
    相等——同一部番劇，排程清單存的 sn 跟下載記錄存的 sn 未必一致（跟
    stats/collector.py 的 sn 對不起來是同一類問題），單純比 sn 會把同一部番劇
    誤判成兩筆、卡片重複出現。"""
    anime_cache = getattr(deps, "anime_cache", None)
    covered_sns = set(schedule_sns)
    if anime_cache is not None and schedule_sns:
        try:
            covered_sns, _origin = anime_cache.resolve_subscription_expansion(list(schedule_sns))
        except Exception:  # noqa: BLE001
            covered_sns = set(schedule_sns)
    extra = []
    for item in _downloaded_anime_list():
        sn = item.get("video_sn")
        if sn is not None and sn in covered_sns:
            continue
        extra.append(item)
    return extra
...
@browse_bp.route("/ui/layout-mode", methods=["POST"])
def set_layout_mode():
    """手機／電視／電腦排版模式的手動覆寫，同樣存進 SettingsStore。"""
    value = request.form.get("value")
    if value in _LAYOUT_MODES:
        current_app.config["DEPS"].settings.update({"web_layout_mode": value})
    return ("", 204)
