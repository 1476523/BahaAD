...
_LOGIN_EXEMPT_ENDPOINTS = {
    ...
    "subscriber_auth.link_remove",
    "subscriber_auth.notify_test",
    "subscriber_auth.notify_test_confirm",
...
# 選擇性子系統各自補充自己的公開模式白名單（套件不存在就跳過，不留痕跡）——
# 名單內容收在各套件自己的 `__init__.py`，這裡不重複列出。
for _module_name in ("bahaad.playback_guard", "bahaad.ad_promo"):
    try:
        _PUBLIC_MODE_ENDPOINTS |= import_module(_module_name).PUBLIC_MODE_ENDPOINTS
    except ImportError:
        pass
...
class WebDeps:
    ...
    # 選擇性子系統用的同步 HTTP client——跟 access_gate_http 故意不共用：那個是
    # GamerSession，這裡是使用者在瀏覽器裡當場觸發的同步請求，不該跟真正的動畫瘋
    # 下載交握搶同一把請求鎖（見 app_shell.py `_UtilityHttpClient` 的說明）。
    # 可選、預設 None。
    ad_report_http: Any = field(default=None)
    # 選擇性子系統用的背景輪詢物件。可選、預設 None。
    ad_campaign_poller: Any = field(default=None)
    # 選擇性子系統用的轉檔快取。可選、預設 None。
    ...
    # subscriber/dispatch.py 的 SubscriberNotifier 單例——app_shell.py 建構，跟
    # gossip_watcher／completion_watch 等排程器共用同一個實例。「我的收藏」帳號連結
    # 區塊的「發送測試訊息」鈕用（使用者 2026-09-23）。可選、預設 None。
    subscriber_notifier: Any = field(default=None)
    # subscriber/verify.py 的 SubscriberVerifyCodes 單例——「發送測試訊息」的一次性
    # 驗證碼關卡（使用者 2026-09-23：要求真的輸入收到的驗證碼才算確認，不是只看
    # 中繼 API 回應「送出成功」）。純記憶體、不落地，可選、預設 None。
    subscriber_verify_codes: Any = field(default=None)
    ...
    # 選擇性子系統用的狀態容器。純記憶體、每個 WebDeps 自動各自一份，不用像
    # hls_cache 特地從 app_shell.py 傳進來。套件不存在（公開原始碼建置）時
    # `PlaybackTokenStore` 是 None，這裡就給 None——反正對應的路由整組也不會
    # 註冊，不會有人真的去用到這個屬性。
...
def create_app(deps: WebDeps) -> Flask:
    ...
    # 選擇性子系統的 blueprint——套件不存在（公開原始碼建置）時選擇性註冊，缺了
    # 就整個安靜不啟用，不影響其他功能。
    for _bp_module, _bp_name in (
        ("bahaad.ad_promo.routes", "ad_promo_bp"),
        ("bahaad.playback_guard.routes", "playback_guard_bp"),
    ):
        try:
            app.register_blueprint(getattr(import_module(_bp_module), _bp_name))
        except ImportError:
            pass
    ...
    def _inject_ui_shell_prefs():
        ...
        if getattr(g, "public_readonly", False):
            ...
            layout_mode = request.cookies.get("bahaad_pub_layout_mode")
            layout_mode = layout_mode if layout_mode in ("auto", "mobile", "tv", "desktop") else "desktop"
            return {
                "ui_theme": theme if theme in ("dark", "light") else "dark",
                # 電視模式側欄一律不收合（使用者 2026-09-21：遙控器操作，72px 窄條
                # 版對電視沒意義，見下面 style.css `[data-layout-mode="tv"]` 說明）。
                "ui_sidebar_collapsed": False if layout_mode == "tv" else request.cookies.get("bahaad_pub_sidebar") == "1",
                "ui_layout_mode": layout_mode,
                "subscribe_intro_seen": True,
            }
        if not _can_read():
            return {
                "ui_theme": "dark", "ui_sidebar_collapsed": False, "ui_layout_mode": "desktop",
                "subscribe_intro_seen": True,
            }

        deps = current_app.config["DEPS"]
        # 使用者 2026-09-21：預設「電腦」，不是「自動」——自動判斷（(pointer: coarse)）
        # 就是智慧電視被誤判成手機排版的根源，沒設過這個偏好的人（含全新安裝）
        # 一律先當電腦看待，真的是手機的人自己點一次切換鈕即可。
        layout_mode = deps.settings.get("web_layout_mode", "desktop")
        return {
            "ui_theme": deps.settings.get("web_theme", "dark"),
            # 電視模式側欄一律不收合（同上，遙控器操作，72px 窄條版對電視沒意義）。
            "ui_sidebar_collapsed": False if layout_mode == "tv" else bool(deps.settings.get("web_sidebar_collapsed", False)),
            "ui_layout_mode": layout_mode,
