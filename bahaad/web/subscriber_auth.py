...
def me():
    ...
    # 於目前登入用的那個管道）。使用者 2026-09-21：改成真的成功送出過一次通知
    # 就不再顯示——`has_successful_notify()` 查的是這個管道自己的身分 id（`linked`
    # 展開後那一筆，不是連結後的「主要」身分 id，`log_notify_history()` 寫的也是
    # 送信當下那個管道身分自己的 id，兩邊要對齊才查得到）。查不到 Bot 資訊（中繼
    # 連不上／沒設定）不影響要不要提醒，只影響提醒文字能不能點連結。
    ...
    telegram_start_reminder = (
        ...
        and not deps.subscriber_store.has_successful_notify(telegram_identity["id"])
    )
