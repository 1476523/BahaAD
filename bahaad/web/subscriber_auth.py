...
@subscriber_auth_bp.route("/subscriber/link/remove", methods=["POST"])
def link_remove():
    """移除帳號連結（使用者 2026-09-23：「帳號連接的部分加入移除帳號連接的選項」）
    ——`channel` 只是用來從目前登入者自己的 `linked_identities_for()` 清單裡找出
    要移除的那一筆，不接受任意身分 id（同檔頭「絕對不能讓使用者自己用網址參數
    指定要併入哪個身分 id」的理由，這裡對稱：也不能指定要拆掉『別人』的連結）。
    `identity`（目前登入解析出來的主要身分）本身不能被選成移除目標，不然等於
    要拆自己這個主要身分，沒有意義。"""
    deps = current_app.config["DEPS"]
    identity = current_subscriber(deps)
    if identity is None:
    ...
    target = next(
        (i for i in deps.subscriber_store.linked_identities_for(identity["id"])
         if i["channel"] == channel and i["id"] != identity["id"]),
        None,
    )
    if target is None:
        return form_result("找不到這個已連結的管道", endpoint="subscriber_auth.me", ok=False)
    deps.subscriber_store.unlink_identity(identity["id"], target["id"])
    return form_result("已移除帳號連結", endpoint="subscriber_auth.me")


@subscriber_auth_bp.route("/subscriber/notify-test", methods=["POST"])
def notify_test():
    """「發送測試訊息」鈕（使用者 2026-09-23）——見 `subscriber/dispatch.py`
    `SubscriberNotifier.send_test_message()` 的說明。`channel` 一樣只用來從目前
    登入者自己的已連結管道裡找收件對象，不接受任意身分 id。只有這顆 JS 按鈕會打
    這支（不是 `<form>`），直接回 JSON，不走 `form_result()` 的無 JS 退路。

    這裡先用 `subscriber_verify_codes` 發一組新的一次性驗證碼、塞進訊息送出，
    訂閱者要把看到的碼貼回 `notify_test_confirm()` 核對才算真的確認收到——單這支
    送出成功只代表「中繼 API 回應成功」，不代表訊息真的送達使用者本人。"""
    deps = current_app.config["DEPS"]
    identity = current_subscriber(deps)
    if identity is None:
        return jsonify({"ok": False, "info": "請先登入"}), 401
    channel = request.form.get("channel")
    target = next(
        (i for i in deps.subscriber_store.linked_identities_for(identity["id"]) if i["channel"] == channel),
        None,
    )
    if target is None:
        return jsonify({"ok": False, "info": "找不到這個管道"})
    notifier = deps.subscriber_notifier
    verify_codes = deps.subscriber_verify_codes
    if notifier is None or verify_codes is None:
        return jsonify({"ok": False, "info": "這台安裝實例尚未完成中繼綁定，暫時無法發送測試訊息"})
    code = verify_codes.issue(target["id"])
    ok, message = notifier.send_test_message(target, code)
    return jsonify({"ok": ok, "info": message})


@subscriber_auth_bp.route("/subscriber/notify-test/confirm", methods=["POST"])
def notify_test_confirm():
    """核對「發送測試訊息」附帶的驗證碼（使用者 2026-09-23）。`channel` 一樣只從
    目前登入者自己的已連結管道裡找目標；驗證通過才呼叫 `mark_verified()`，讓
    `me()` 的「請先傳一次 /start」之類提醒下次重新整理就會消失（前端收到 `ok:
    true` 才會真的重新整理頁面，見 `subscriber_me.js`）。"""
    deps = current_app.config["DEPS"]
    identity = current_subscriber(deps)
    if identity is None:
        return jsonify({"ok": False, "info": "請先登入"}), 401
    ...
    if target is None:
        return jsonify({"ok": False, "info": "找不到這個管道"})
    verify_codes = deps.subscriber_verify_codes
    if verify_codes is None or not verify_codes.verify(target["id"], code):
        return jsonify({"ok": False, "info": "驗證碼錯誤或已過期，請重新發送測試訊息"})
    deps.subscriber_store.mark_verified(target["id"])
    return jsonify({"ok": True, "info": "驗證成功"})
...
def me():
    ...
    # 於目前登入用的那個管道）。使用者 2026-09-23：改成要真的核對過「發送測試
    # 訊息」附帶的驗證碼才算數（`store/subscribers.py` 的 `is_verified()`／
    # `mark_verified()`），不是單看中繼 API 回應「送出成功」——查的是這個管道
    # 自己的身分 id（`linked` 展開後那一筆，不是連結後的「主要」身分 id）。查不到
    # Bot 資訊（中繼連不上／沒設定）不影響要不要提醒，只影響提醒文字能不能點連結。
    telegram_bot_username = None
    telegram_identity = next((i for i in linked if i["channel"] == "telegram"), None)
    telegram_start_reminder = (
        telegram_identity is not None and not deps.subscriber_store.is_verified(telegram_identity["id"])
    )
    ...
    return render_template(
        ...
        telegram_bot_username=telegram_bot_username, telegram_start_reminder=telegram_start_reminder,
...
def logout():
    """登出後導回訂閱列表頁（使用者 2026-09-17：不要停在「訂閱者登入」頁——那是
    要登入才會特地去的頁面，登出這個動作本身沒有理由把人留在那裡，跟擁有者登出
    導回登入頁的語意不一樣：訂閱者登出後應該回到公開模式原本在看的唯讀列表）。"""
    deps = current_app.config["DEPS"]
    token = request.cookies.get(SESSION_COOKIE)
    if token and deps.subscriber_store is not None:
        deps.subscriber_store.delete_session(token)
    response = redirect(url_for("browse.subscriptions"))
...
def _stats_sync_subscription(deps, canonical_sn: int, title: str | None) -> None:
    ...
    清單共用同一個 `stats_collector.record_subscription()`。人數當場用
    `subscription_count()` 重新算（擁有者排程訂閱 + 各訂閱者身分追蹤數相加，使用者
    2026-09-16：「番劇訂閱數」該是擁有者跟各訂閱者身分分別累加，不是這台裝置只回報
    「有沒有人在追蹤」的 0/1），不能只看這次 follow/unfollow 的結果覆蓋——這個 sn
    可能同時也在擁有者的排程清單裡、也可能還有其他訂閱者身分在追蹤。"""
    collector = getattr(deps, "stats_collector", None)
    if collector is None:
        return
    try:
        collector.record_subscription(canonical_sn, title, count=subscription_count(deps, canonical_sn))
