...
//
// 一般更新（policy="general"）：進頁面就彈一次提醒視窗，之後每 60 秒再問一次
// /update/pending，有更新且視窗還沒開著就彈。「稍後再說」只關掉視窗，下次換頁或滿
// 60 秒又彈。頂列黃色工具更新鈕點下去也能叫出視窗。
// 重點更新（policy="important"）：整個網頁 UI 已被鎖在首頁（後端 before_request 硬回
// 503），不自動彈視窗；頂列紅色工具更新鈕點下去可以叫出「立即套用」視窗。
//
// 差異檔在背景下載完成時（`_pending_update` 從無變有）：橫幅原本是伺服器 render 的，
// 開著的分頁不會自己變。輪詢一偵測到就把「背景下載中」橫幅換成「套用更新並重新啟動」
// 橫幅、順便彈提醒視窗，不用使用者自己重新整理（使用者 2026-09-09）。
(function () {
  "use strict";
  ...
  function bannerText(pending) {
    ...
    if (pending.policy === "important") {
      t = "重點更新：必須套用才能繼續使用 BahaAD（" + (pending.version || "") + "）";
    } else {
      t = "有" + (pending.minor ? "小" : "") + "更新可套用（" + (pending.version || "") +
          "），建議立即套用並重新啟動";
    }
