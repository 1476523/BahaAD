...
(function () {
  ...
  function ensureClones() {
    ...
    // 只延後「封面圖」（有 data-cache-src 的那些）——鈴鐺／重新檢查等小圖示是普通
    // 靜態 src，clone 也照樣留著，不然捲到複製輪時鈴鐺會變破圖框（使用者 2026-09-10
    // 「往上看訂閱鈴鐺消失」）。
    var originalImgs = cycle.querySelectorAll("img[data-cache-src]");
    [before, after].forEach(function (c) {
      ...
      var clonedImgs = c.querySelectorAll("img[data-cache-src]");
