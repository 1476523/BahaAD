...
(function () {
  ...
  // 手機／小平板（≤ 48rem）：同一顆鈕改成開關「滑出式抽屜」（.sidebar-drawer-open），
  // 不寫進偏好（抽屜是一次性的）。桌面維持原本的「收合 72px」＋存偏好行為。
  var MOBILE_MQ = window.matchMedia("(max-width: 48rem)");
  ...
  function initSidebarToggle() {
    ...
    // 轉回桌面尺寸時把抽屜狀態清掉
    var onMqChange = function () { if (!MOBILE_MQ.matches) { closeDrawer(); } };
