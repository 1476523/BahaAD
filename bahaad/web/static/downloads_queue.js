...
(function () {
  ...
  var lastDownloadedHtml = downloadedSection ? downloadedSection.innerHTML : null;
  function refreshDownloadedSection() {
    if (!downloadedSection) return;
    fetch("/browse/downloads/downloaded")
      .then(function (r) { return r.ok ? r.text() : null; })
      .then(function (html) {
        // 內容沒變就別重繪——不然每 2 秒把整區 innerHTML 換掉，封面 <img> 全部重新
        // 被 img_loader 處理一輪（換頁後 data-cache-src → 重打 /cache/img-batch），
        // 畫面一直閃（使用者 2026-09-09「下載列表一直重複刷新」）。
        if (html !== null && html !== lastDownloadedHtml) {
          lastDownloadedHtml = html;
          downloadedSection.innerHTML = html;
        }
      })
      .catch(function () {});
  }
