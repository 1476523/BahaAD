...
(function () {
  ...
  function initPlayer() {
    ...
    // 投放（Chromecast／Remote Playback API）修正——使用者 2026-09-25 回報公開模式
    // 播放器缺少投放鈕；比對 admin／訪客兩種身分的 `data-remote-playback` 屬性後
    // 確認：遠端模式下桌面瀏覽器用 hls.js 把串流餵進 <video>，餵進去的其實是瀏覽器
    // 記憶體內的 blob: 網址，投放裝置沒辦法獨立抓取，瀏覽器判斷「沒有可投放的
    // 東西」，鈕永遠不會出現。修法：另外準備一個不接上 hls.js、只設定真正網路
    // 位址（.m3u8，含 token）的隱藏 <video> 元素，專門讓 Remote Playback API
    // 偵測／投放用——畫面上真正播放的 <video> 完全不受影響，照常用 hls.js。投放
    // 裝置（電視／Chromecast）通常原生支援 HLS，不需要靠這台瀏覽器幫忙解碼。
    // **實驗性修正**：沒有實體投放裝置可以實測，理論上應該可行但未經實機驗證。
    var castVideo = remote ? document.createElement("video") : null;
    if (castVideo) {
    ...
    function load(sn, label) {
      ...
      fetch("/anime/episode/" + sn + "/hls/token", { method: "POST" })
        ...
        .then(function (data) {
          ...
          // 讓投放用的隱藏 <video> 指向這個真正的網路位址（見上面 castVideo 的
          // 說明）——不管等下走 hls.js 還是原生 HLS 分支，投放鈕看到的都是這個。
          if (castVideo) castVideo.src = m3u8;
    ...
    overlay.querySelectorAll("[data-player-act]").forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        if (a === "back") step(-SKIP);
          ...
          var cv = castVideo || video;
          if (cv.remote) cv.remote.prompt().catch(function () {});
        }
      });
    });
    ...
    // 遠端模式（castVideo 存在）看隱藏投放專用 <video> 的可用性，不是畫面上那個
    // 接了 hls.js 的——理由見上面 castVideo 的說明。
    var castBtn = overlay.querySelector('[data-player-act="cast"]');
    var castRemoteSource = castVideo || video;
    if (castBtn && castRemoteSource.remote && typeof castRemoteSource.remote.watchAvailability === "function") {
      castRemoteSource.remote
