...
from bahaad.net.proxy import build_proxy_selector, send_with_failover
...
from bahaad.scheduler.version_check import VersionChecker, _strip_v
from bahaad.scheduler.youranimes_sync import YourAnimesSync
from bahaad.stats.activity import StatsActivity
from bahaad.stats.collector import StatsCollector, resync_all_subscriptions
...
from bahaad.store.subscriber_relay import SubscriberRelayStore
from bahaad.store.subscribers import SubscribersStore
from bahaad.subscriber.dispatch import SubscriberNotifier
from bahaad.subscriber.verify import SubscriberVerifyCodes
...
from bahaad.updater import UPDATE_EXTRACTED_DIRNAME, UPDATE_STAGING_DIRNAME
...
class Services:
    ...
    web_deps: WebDeps
    # 選擇性子系統的背景輪詢物件——公開建置沒有對應套件時是 None，見
    # build_services() 的 try/except ImportError。型別故意不寫死，避免這個核心
    # 檔案需要 import 選擇性套件的型別名稱。
    ad_campaign_poller: object | None = None


class _UtilityHttpClient:
    """給版本檢查／差異更新下載／access_gate_server 心跳這類「打自家中繼或
    GitHub、完全不需要動畫瘋登入 cookie」的呼叫用——獨立的乾淨 curl_cffi
    session，不透過 `GamerSession`（比照 `YourAnimesFetcher` 的既有模式，見
    `docs/requirements/gamer_client_session.md`「唯一刻意的例外」一節）。

    這裡本來也是沿用 `GamerSession`（理由是「多帶一個對第三方無意義但無害的
    Referer 標頭，不值得為了語意純粹另外開一個乾淨 session」），但這個判斷是在
    差異更新檔案下載的逾時只有 30 秒時做的——那時候 `GamerSession.get()` 內部的
    `_request_lock` 最多被單一檔案下載卡住 30 秒。使用者 2026-09-15 回報下載大檔
    （`BahaAD.exe`）常常沒下載完就被 30 秒總時限判定逾時失敗，逾時被放寬到 600
    秒後，同一把鎖現在可能被單一檔案下載卡住到 10 分鐘（重試 3 次更是到 30
    分鐘）——使用者 2026-09-16 實測抓到：更新檔正在背景下載時，排行榜／隱私權
    政策的統計查詢（也是透過同一個 `session` 打 access_gate_server）卡了 43 秒
    才回應，根因就是被同一把鎖擋住。獨立開一個 session 徹底解決，不會再有任何
    背景更新/心跳流量跟動畫瘋的真實下載交握互搶這把鎖。"""

    def __init__(self, proxy_selector) -> None:
        self._session = curl_requests.Session(impersonate="chrome")
        self._proxy_selector = proxy_selector

    def get(self, url: str, **kwargs):
        return send_with_failover(self._session.get, self._proxy_selector, url, **kwargs)
...
def build_services(base_dir: Path) -> Services:
    ...
    # 版本檢查／差異更新下載／access_gate_server 心跳＋統計查詢共用這個獨立 session
    # （見 `_UtilityHttpClient` 的說明）——不透過 `GamerSession`，不會被大檔案下載
    # 卡住的鎖擋住其他真的在等這把鎖的動畫瘋請求。
    utility_http = _UtilityHttpClient(proxy_selector)
    # `UpdateCoordinator` 的 `download_zip()` 多線程分段下載整包更新 zip 時，每條
    # 執行緒要有自己獨立的 curl_cffi Session（不能共用 `utility_http` 那一個——Session
    # 不保證能被多條執行緒同時呼叫）。這個 factory 每次呼叫都建一個全新的
    # `_UtilityHttpClient`，供 `fetcher.py` 的每條分段下載執行緒各自使用一次。
    update_zip_http_factory = lambda: _UtilityHttpClient(proxy_selector)  # noqa: E731
    ...
    stats_query = StatsQuery(utility_http, _ACCESS_GATE_SERVER_BASE_URL)
    ...
    # `no_cookies=True`）——共用一個無 cookie session。`serialize_requests=False`：片段是
    # `max_concurrent_segments` 條執行緒平行抓的，這個 session 不能加請求鎖，不然會退化成
    # 單執行緒下載（使用者 2026-09-09）；無 cookie CDN 請求也沒有 cookie jar 競態問題。
    cdn_session = GuestSession(
        identity_store, proxy_selector=proxy_selector, serialize_requests=False
    )
    ...
    playlist = PlaylistClient(
        ...
        # video_src.php 回帳號／cookie 錯誤時，先跑一次登入保活瀏覽再重試（使用者
        # 2026-09-10：「前段時間才下載完、不會閒置太久」——閒置沒真死的 session 這樣救回來，
        # 不用等 6 小時後的背景完整檢查或使用者手動重登）。
        on_auth_recover=cookie_rotation.warm_up_session,
    )
    ...
    # 訂閱者 Discord/Telegram 通知中繼（subscriber_notify.md，Phase 1）：跟
    # access_gate_server 是同一個中繼服務，共用 _ACCESS_GATE_SERVER_BASE_URL，不用
    # 再另外定義一個網址常數。
    subscriber_relay_store = SubscriberRelayStore(database)
    subscriber_store = SubscribersStore(database)
    ...
    # 訂閱者 Discord/Telegram 推播（見「真正的推播發送」筆記）：跟擁有者的
    # notify_store 完全獨立。下載成功時 main_loop 通知；公告（暫停/延後/加更/下架）、
    # 番劇完結、每日新番通知（個人化版）分別由下面的 gossip_watcher／completion_watcher
    # 通知——同一個 notifier 實例，各處只是多呼叫不同方法（見 subscriber/dispatch.py）。
    subscriber_notifier = SubscriberNotifier(
        subscriber_store, subscriber_relay_store, session, _ACCESS_GATE_SERVER_BASE_URL, settings,
        anime_cache=anime_cache,
    )
    # 「發送測試訊息」的一次性驗證碼關卡（使用者 2026-09-23）。純記憶體單例，
    # 見 subscriber/verify.py 開頭說明。
    subscriber_verify_codes = SubscriberVerifyCodes()
    ...
    version_checker = VersionChecker(
        settings, http=utility_http, current_version=__version__, on_new_version=_kick_diff_update,
    )
    ...
    update_coordinator = UpdateCoordinator(
        ...
        diagnostics=diagnostics_reporter,
        http_factory=update_zip_http_factory,
    )
    ...
    # 選擇性子系統掛勾——套件不存在時這兩個物件都是 None，呼叫端一律用
    # getattr(..., None) 容錯，不影響其他功能。啟動時先確保這個子系統需要的
    # 樣板／靜態檔都在（差異更新升級上來的安裝可能缺，見 ad_promo/__init__.py
    # `ensure_bundled_assets()` 開頭說明），再建立背景輪詢物件。
    try:
        import_module("bahaad.ad_promo").ensure_bundled_assets()
    except ImportError:
        pass

    try:
        ad_hls_cache = import_module("bahaad.ad_promo.ad_hls").AdHlsCache(base_dir / "ad_hls_cache", find_ffmpeg())
    except ImportError:
        ad_hls_cache = None

    try:
        ad_campaign_poller = import_module("bahaad.ad_promo.campaigns").CampaignPoller(
            utility_http, _ACCESS_GATE_SERVER_BASE_URL, cache_dir=base_dir / "ad_promo_media_cache",
            ad_hls_cache=ad_hls_cache,
        )
    except ImportError:
        ad_campaign_poller = None
    ...
    web_deps = WebDeps(
        ...
        access_gate_server_base_url=_ACCESS_GATE_SERVER_BASE_URL,
        ad_report_http=utility_http,
        ad_campaign_poller=ad_campaign_poller,
        ad_hls_cache=ad_hls_cache,
        ...
        # 帶登入 cookie 的 session——只有首頁本季新番／週期表爬取會在偵測到動畫瘋
        # 登入態時改用它，好把訪客看不到的年齡限制（18 禁）番劇補進清單（使用者
        # 2026-09-10）。見 web/anime_data.py `_home_http()`。
        browse_member_http=session,
        notify_store=notify_store,
        notify_http=session,
        subscriber_relay_store=subscriber_relay_store,
        subscriber_relay_http=session,
        subscriber_relay_server_base_url=_ACCESS_GATE_SERVER_BASE_URL,
        subscriber_store=subscriber_store,
        subscriber_notifier=subscriber_notifier,
        subscriber_verify_codes=subscriber_verify_codes,
        gossip_store=gossip_store,
    ...
    # 開機把目前所有訂閱狀態（擁有者排程 + 各訂閱者身分）重新同步進即時統計一次
    # （使用者 2026-09-16 實測：排行榜「番劇訂閱數」完全是空的——這個功能上線前
    # 就已經存在的訂閱、或透過排程清單文字編輯器整批修改的項目，從沒觸發過
    # record_subscription()，永遠不會出現在統計裡）。純本機 DB 查詢，很快，不影響
    # 啟動時間；見 bahaad/stats/collector.py 的 resync_all_subscriptions() 註解。
    try:
        resync_all_subscriptions(web_deps)
    except Exception:  # noqa: BLE001 - 埋點不能拖垮啟動流程
        logger.debug("stats: 開機 resync_all_subscriptions 失敗", exc_info=True)
...
def start_background_services(services: Services) -> None:
    ...
    if services.ad_campaign_poller is not None:
        services.ad_campaign_poller.start()
...
def stop_background_services(services: Services) -> None:
    ...
    # 讓遷移背景執行緒盡快結束當前這輪迴圈（不中斷正在進行的單筆複製）——沒跑完的
    # 部分下次啟動 resume_pending_migration() 會接著跑，見 download_dir_migration.md。
    services.main_loop.stop_migration_thread()
    services.main_loop.stop()
    services.diagnostics_reporter.stop()
    services.stats_reporter.stop()
    services.stats_heartbeat.stop()
    if services.ad_campaign_poller is not None:
        services.ad_campaign_poller.stop()
    ...
    def _start_apply(icon: pystray.Icon) -> None:
        ...
        try:
            launch_apply_and_restart(
                install_root=updater_install_root(),
                staging_dir=base_dir / UPDATE_STAGING_DIRNAME / UPDATE_EXTRACTED_DIRNAME,
        ...
        def _work() -> None:
            ...
            try:
                ...
                if latest:
                    ctypes.windll.user32.MessageBoxW(
                        None,
                        f"發現新版本 v{_strip_v(latest)}，已開始在背景下載更新檔。\n\n"
...
def run() -> None:
    ...
    # 更改下載目錄時自動遷移既有檔案（docs/requirements/download_dir_migration.md）：
    # 上次遷移沒跑完就被關掉——`resume_pending_migration()` 是輕量的 settings 讀取，
    # 真的有殘留旗標才會啟動背景執行緒接著跑。
    services.main_loop.resume_pending_migration()
    ...
    # 使用者 2026-09-13 回報：`waitress` 的 asyncore 事件迴圈在 Windows 上偶爾會炸
    # `OSError: [WinError 10038]`（`select()` 碰到一個已經失效的 socket handle，常見
    # 觸發源是防毒/過濾軟體強制中斷某條連線）。`waitress.server.run()` 只接
    # `SystemExit`/`KeyboardInterrupt`，其他例外一律往外炸——原本這裡是
    # `threading.Thread(target=server.run)`，這個背景執行緒死掉以後**整個網頁伺服器
    # 從此再也不接受任何連線**（只有這條例外會被 `threading.excepthook` 記進日誌，
    # 使用者完全看不出網頁為什麼突然打不開／像是登入態失效，其實是伺服器整個停了），
    # 只能靠使用者自己發現後手動重啟工具。改成外面包一層迴圈：run() 意外死掉就整個
    # 重新建一個 server（舊的 map 裡可能還卡著那個壞掉的 socket，不能沿用同一個
    # map 重跑，不然會立刻用同一顆壞 fd 再炸一次）、關掉舊的監聽 socket 再繼續服務，
    # 不會因為這個背景執行緒死掉就讓整個網頁介面永久斷線。
    _server_holder = [_make_server()]
    _server_lock = threading.Lock()

    def _run_server_forever() -> None:
        while True:
            with _server_lock:
                current = _server_holder[0]
            try:
                current.run()
            except (SystemExit, KeyboardInterrupt):
                raise
            except Exception:  # noqa: BLE001 - 背景服務迴圈，任何例外都要重開而不是死掉
                logging.getLogger("bahaad").critical(
                    "web 伺服器背景執行緒發生未預期例外，正在重新啟動監聽（背景下載／排程不受影響）",
                    exc_info=True,
                )
                try:
                    current.close()
                ...
                time.sleep(1.0)
                with _server_lock:
                    _server_holder[0] = _make_server()
                continue
            else:
                break  # 正常關閉（外面呼叫 server.close() 之後 run() 收工）

    server_thread = threading.Thread(target=_run_server_forever, daemon=True)
    server_thread.start()
    ...
    # server_thread 是 daemon、關閉看門狗 10 秒後 os._exit 保底。關的是**目前這一顆**
    # server——如果背景執行緒中途因為上面那個 WinError 10038 重開過，`_server_holder[0]`
    # 已經是新的那顆，不能關舊的（舊的早就 close() 過了）。
    try:
        with _server_lock:
            _server_holder[0].close()
    except Exception:  # noqa: BLE001
        pass
