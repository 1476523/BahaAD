...
class Services:
    ...
    web_deps: WebDeps
    # 動態廣告活動背景輪詢（使用者 2026-09-19）——整包在 `bahaad.ad_promo`（不進
    # 公開 repo），公開建置沒有這個套件時是 None，見 build_services() 的
    # try/except ImportError。型別故意不寫死 `CampaignPoller`，避免這個核心檔案
    # 需要 import ad_promo 套件的型別名稱。
...
def build_services(base_dir: Path) -> Services:
    ...
    # 廣告嵌入 HLS 串流（使用者 2026-09-20）——選擇性 import，套件不存在時
    # `ad_hls_cache` 是 None，play_episode_hls() 就不插入廣告，播放清單維持純正片。
    # 使用者 2026-09-19：這裡要先建好，才能傳給下面的 `CampaignPoller`，讓它
    # 每輪成功拉到活動清單時，順便清掉這裡過期活動的轉檔快取。
    try:
        from bahaad.ad_promo.ad_hls import AdHlsCache

        ad_hls_cache = AdHlsCache(base_dir / "ad_hls_cache", find_ffmpeg())
    except ImportError:
        ad_hls_cache = None

    # 動態廣告活動背景輪詢（使用者 2026-09-19）——整包在 `bahaad.ad_promo`（不進
    # 公開 repo，見該套件說明），選擇性 import，套件不存在時 `ad_campaign_poller`
    # 就是 None，web/ad_promo 那邊的呼叫端本來就用 getattr(..., None) 容錯。
    try:
        from bahaad.ad_promo.campaigns import CampaignPoller

        ad_campaign_poller = CampaignPoller(
