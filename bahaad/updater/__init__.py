...
# 2026-09-25 改版：`policy.py` 把下載好的整包更新 zip 解壓縮到
# `{data_dir}\update_staging\extracted\`——跟 zip 本身放同一層 `update_staging` 底下，
# 但獨立一個子資料夾，`apply_update.ps1` 的 robocopy 來源要指到這裡（解壓縮後的完整
# 安裝內容），不是 zip 檔案本身所在的那一層。
UPDATE_EXTRACTED_DIRNAME = "extracted"
