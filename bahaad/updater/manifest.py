...
class UpdateManifest:
    ...
    mandatory: bool
    notes: str
    # 2026-09-25 改版：不再逐檔列差異，manifest 只記整包更新 zip（GitHub Release 附件）
    # 的下載網址／雜湊／大小——原因見 docs/decisions/0001（當日附加章節）：差異更新原本
    # 用的 raw.githubusercontent.com 實測完全不支援 HTTP Range，沒辦法分段續傳/加速；
    # Release 附件走的 CDN 有支援，`updater/fetcher.py` 的 `download_zip()` 靠這個做
    # 多線程分段下載＋斷點續傳。
    zip_url: str
    zip_sha256: str
    zip_size: int
...
def fetch_manifest(http: HttpGetter) -> UpdateManifest:
    ...
    更新流程，不繼續往下走。`timeout`：連不到 raw.githubusercontent.com 時盡快放棄
    （使用者的 VPN／過濾軟體擋掉時，舊版預設會卡 ~22 秒才 curl(28)）。"""
    response = http.get(MANIFEST_URL, timeout=15)
