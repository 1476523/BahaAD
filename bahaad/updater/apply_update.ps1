...
# Mirror the staged (already-extracted) install tree over the install directory.
# 2026-09-25: the staging dir used to hold only the diffed files, so a merge (/E) was
# correct -- anything not in staging was left untouched on purpose. Now it holds a full
# extracted release zip, so we switch to /MIR (mirror): it still adds/overwrites files
# from the new version, but also deletes files present in InstallDir that are no longer
# in the new version (renamed/removed modules from an old release don't linger forever).
# robocopy exit codes 0-7 are success, 8+ is a real failure (e.g. a file still locked
# because the old process has not fully exited).
robocopy $StagingDir $InstallDir /MIR /NFL /NDL /NJH /NJS /NP /R:3 /W:2 | Out-Null
$rc = $LASTEXITCODE
if ($rc -ge 8) {
