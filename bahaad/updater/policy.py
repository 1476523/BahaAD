...
from bahaad.updater import UPDATE_EXTRACTED_DIRNAME
from bahaad.updater.fetcher import HashMismatchError, download_zip, extract_zip
...
class UpdateCoordinator:
    def __init__(
        ...
        http_factory: Callable[[], HttpGetter] | None = None,
    ) -> None:
        self._settings = settings
        self._http = http
        # 2026-09-25：`download_zip()` 多線程分段下載時，每條執行緒要有自己獨立的
        # `HttpGetter`（curl_cffi 的 Session 不保證能被多條執行緒同時呼叫）。沒給
        # `http_factory` 就退回每次都回傳同一個 `http`——開發模式／測試沒有真的並行
        # 下載時這樣完全沒問題；正式環境（`app_shell.py`）會傳真正每次建一個新
        # session 的 factory。
        self._http_factory = http_factory or (lambda: http)
        ...
        # `on_update_ready(version: str, policy_value: str)`：更新 zip 剛下載＋解壓縮完、
        # `_pending_update` 從「這個版本還沒下載」變成「已就緒」時呼叫一次（app_shell
        # 掛系統匣氣泡通知——使用者 2026-09-08：「好歹也提示說有更新」）。同一版本只
        # 通知一次。
    ...
    def check_once(self) -> UpdatePolicy | None:
        ...
        try:
            ...
            conn = classify_connection_error(exc)
            if conn is not ConnectionStatus.OTHER and not isinstance(exc, ManifestSignatureError):
                # 純粹連不到 GitHub（逾時／斷線／DNS）——不是更新機制故障。使用者的網路
                # 環境（VPN／過濾軟體）長期擋掉 raw.githubusercontent.com 時，每輪一次的
                # WARNING + 診斷回報太吵。記 INFO（不寫檔案 log／不進日誌頁）、不回報。
                logger.info("updater 這輪查不到 manifest（%s），下輪再試", conn.value)
                return None
        ...
        # 已經下載過「同一版、同一種更新等級」、而且解壓縮出來的內容還在 → 不用每輪
        # 重抓（1 小時一輪，重抓整包 zip 會很浪費）。版本或等級變了（維護者重發、或把
        # general 升成 mandatory）、或暫存內容不見了（套用失敗、被外部清掉）才往下
        # 重抓、更新旗標。
        extracted_dir = self._staging_dir / UPDATE_EXTRACTED_DIRNAME
        pending = self._settings.get(_SETTINGS_KEY)
        if (
            pending
            and pending.get("version") == manifest.version
            and pending.get("policy") == policy.value
            and extracted_dir.is_dir()
            and any(extracted_dir.iterdir())
        ):
            return policy

        max_retries = self._settings.get("max_retries", _DEFAULT_MAX_RETRIES)
        zip_path = self._staging_dir / "update.zip"
        started_at = time.monotonic()
        try:
            download_zip(
                self._http, self._http_factory, manifest.zip_url, manifest.zip_sha256,
                manifest.zip_size, zip_path, max_retries=max_retries,
            )
            extract_zip(zip_path, extracted_dir)
        except Exception as exc:
            integrity = _contains_hash_mismatch(exc)
            conn = classify_connection_error(exc)
            if conn is not ConnectionStatus.OTHER and not integrity:
                # 跟上面 fetch_manifest() 失敗那支同一個理由（使用者 2026-09-24 回報：
                # 更新檔比 manifest.json 大很多、下載時間也長很多，網路一頓一頓的使用者
                # 更容易在這裡撞到純粹的連線問題，不是更新機制故障）——純連不上／逾時／
                # 斷線不算故障，記 INFO（不進診斷、也不驚動 DiagnosticsLogHandler 的
                # 「錯誤都上傳」兜底，那支只聽 ERROR 等級，INFO 不會觸發），等下一輪重試。
                logger.info("updater 這輪下載更新檔案時斷線（%s），下輪再試", conn.value)
                return None
            ...
            self._report(
                ...
                file_integrity_result=FileIntegrityResult.MISMATCH if integrity else None,
            )
            return None
